...
const url = "https://www.whaleex.com/BUSINESS/api/public/v1/ticker/VIGEOS";
...
function write(){
	...
	axios.get(`${url}`)
		.then(results=>{
			...
			if (results.data && results.data.lastPrice){

				console.log(" results.data.lastPrice",  results.data.lastPrice);

				eos.contract(oracleContract)
					.then((contract) => {
						contract.push({
								owner: owner,
								quotes:[{value: parseInt(Math.round(results.data.lastPrice * 100000000)), pair: "vigeos"}]
							},
							{
