...
void vigorlending::assetin(const eosio::name& from, const eosio::name& to, const eosio::asset& assetin, const std::string& memo) {
  ...
  if(from == get_self() || to != get_self() || memo == "donation" || from == eosio::name("vigorsxcurve")) return;
  ...
  if(localAssetIn.symbol == VIGOR_SYMBOL) {
    if(memo == "collateral") {
      ...
      if (symbol_swap(localAssetIn.symbol) == USDT_SYMBOL){ // all USDT deposits are zapped into SXE
        //eosio::check(asset_swap(localAssetIn).amount >= 5.0 * std::pow(10.0, asset_swap(localAssetIn).symbol.precision()), "quantity too small, minimum size 5 USDT");
        ...
        if (itr != curvesx_pairs.end()) {
          ...
          eosio::action(eosio::permission_level {get_self(), eosio::name("active")},
                        symbolIterator->contract,
                        eosio::name("transfer"),
                        std::make_tuple(get_self(), ZAP_CONTRACT, asset_swap(localAssetIn) + usdtbalance, std::string("SXE")))
          .send();
          eosio::asset SXE = eosio::asset(0, SXE_TOKEN_SYMBOL);
          eosio::asset asset = eosio::asset(0, SXE_TOKEN_SYMBOL);
          _g.getCollaterals().hasAsset(SXE, asset);
          SXE += asset;
          asset.amount = 0;
          _g.getInsurances().hasAsset(SXE, asset);
          SXE += asset;
...
void vigorlending::exassetout(const eosio::name& usern, const eosio::asset& assetout, const std::string& memo, const bool& exec, const bool partialupdate, const uint64_t& step, double& totalval, double& totalvalbefore) {
  ...
  if(memo == "borrow" && assetout.symbol == VIGOR_SYMBOL) {
    ...
    eosio::check(user.valueofcol >= mincol * 0.998 * ((user.debt.amount + localAssetOut.amount) / std::pow(10.0, localAssetOut.symbol.precision())),
                 "Max allowable borrow " + eosio::asset(std::pow(10.0, localAssetOut.symbol.precision()) * std::max((user.valueofcol / mincol) - (user.debt.amount / std::pow(10.0, localAssetOut.symbol.precision())), 0.0), localAssetOut.symbol).to_string() +
    ...
    if(exec) {
      ...
      takefee(fee);
      _g.txnfee += fee.amount / std::pow(10.0, fee.symbol.precision());
      localAssetOut -= fee;
    ...
    if(memo == "insurance") {
      ...
      if(exec) {
        ...
        if (symbol_swap(localAssetOut.symbol) == USDT_SYMBOL){ // all USDT withdrawls are zapped out of SXE
          //eosio::check(asset_swap(localAssetOut).amount >= 3.0 * std::pow(10.0, asset_swap(localAssetOut).symbol.precision()), "quantity too small, minimum size 5 USDT");
          // query amount of liquid USDT in vigorlending
          eosio::asset usdtbalance = eosio::asset(0,USDT_SYMBOL);
    ...
    if(exec) {
      ...
      if(lastAssetOut.is_valid() && lastAssetOut.amount > 0) {
        if (localAssetOut.symbol == USDT_SYMBOL){
          eosio::asset usdtbalance = eosio::asset(0,USDT_SYMBOL);
            auto symbolIterator = _whitelist.find(localAssetOut.symbol.raw());
            if(symbolIterator == _whitelist.end()) {
              accounts accounts_table(symbolIterator->contract, get_self().value);
              const auto& itr = accounts_table.find(localAssetOut.symbol.code().raw());
              if(itr != accounts_table.end())
                usdtbalance = itr->balance;
            }
          eosio::check(usdtbalance >= lastAssetOut, "try smaller increments of USDT, slippage too high for large transactions");
        }
        eosio::action(eosio::permission_level {get_self(), "active"_n},
                      symbolIterator->contract,
                      "transfer"_n,
