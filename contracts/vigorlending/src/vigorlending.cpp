...
void vigorlending::openaccount(const eosio::name& owner) {
  ...
  eosio::check(
  owner.value == eosio::name("blazekos.vr").value ||
  owner.value == eosio::name("bonzmanifest").value ||
  owner.value == eosio::name("corts1111111").value ||
  owner.value == eosio::name("cypherbadger").value ||
  owner.value == eosio::name("dougweisberg").value ||
  owner.value == eosio::name("eosjhempot11").value ||
  owner.value == eosio::name("eoswatchdogs").value ||
  owner.value == eosio::name("floridaoneos").value ||
  owner.value == eosio::name("ha4dombsgmge").value ||
  owner.value == eosio::name("maxmaxmaxmac").value ||
  owner.value == eosio::name("mikelennie.n").value ||
  owner.value == eosio::name("petercheneos").value ||
  owner.value == eosio::name("raleighvigor").value ||
  owner.value == eosio::name("sergiomarinm").value ||
  owner.value == eosio::name("stablequan11").value, "Must be custodian of vigordacltd1 to openaccount");
  }
#else
  if(_config.getGateKeeper() == 3) {  // Custodian
...
void vigorlending::sweep() {
  // savings and vigfees is being removed.
  // sweep user savings into l_debt, and vigfees into collateral.
  if (_g.vigfees == 0 && _g.savings == 0.0) {
    _b.usern = _u.begin()->usern;
  } else {
  auto itr = _u.lower_bound(_b.usern.value);
  auto stop_itr = _u.end();
  ...
  while(itr != stop_itr && counter++ < _b.batchsize) {
    ...
    if(amt.amount > 0) {
      _u.modify(user, get_self(), [&](auto& modified_user) {
        modified_user.vigfees -= amt.amount;
    ...
    if (user.savings > 0.0) {
      ...
      _u.modify(user, get_self(), [&](auto& modified_user) {
        modified_user.savings = 0.0;
        modified_user.l_debt += savings;
        _g.l_totaldebt += savings;
      });
    }
    itr++;
  }
...
void vigorlending::collectfee() {
  ...
  eosio::microseconds dmsec(10800000000);
#else
  ...
  while(itr != stop_itr && counter++ < _b.batchsize) {
    ...
    // collect the borrow fee (l_tespay) payment from user l_debt and reduce global l_debt, then give it to vigordacfund (payoff it's debt if any, deposit remainder to l_debt)
    ...
    if (l_tespay_vigor.amount > 0) {
      ...
      eosio::asset l_tespay_payoff = eosio::asset(std::min(l_tespay_vigor.amount, vigordacfund->debt.amount), VIGOR_SYMBOL); // payoff VIGOR debt, up to the amount vigordacfund has
      eosio::asset remainder = l_tespay_vigor.amount > vigordacfund->debt.amount ? eosio::asset(l_tespay_vigor.amount - vigordacfund->debt.amount, VIGOR_SYMBOL) : eosio::asset(0, VIGOR_SYMBOL);
      ...
      if (remainder.amount > 0) {
        _u.modify(vigordacfund, get_self(), [&](auto& modified_user) {
        modified_user.l_debt += remainder;
       _g.l_totaldebt += remainder;
        });
      }
    }
...
void vigorlending::rewardswap(const eosio::name& miner){
  ...
  for(const auto& userCollateral : asset_container_t(*userCollaterals)) {
   ...
   // }
    else if (userCollateral.symbol == IQ_TOKEN_SYMBOL && userCollateral.amount > 100*std::pow(10.0, IQ_TOKEN_SYMBOL.precision()))
      memo = "swap,1,93-11";
    else if (userCollateral.symbol == USDT_SYMBOL && userCollateral.amount > 2*std::pow(10.0, USDT_SYMBOL.precision()))
      memo = "swap,1,12-11";
    else if (userCollateral.symbol == PBTC_TOKEN_SYMBOL && userCollateral.amount > 0.0001*std::pow(10.0, PBTC_TOKEN_SYMBOL.precision()))
      memo = "swap,1,177-11";
    else if (userCollateral.symbol == PETH_TOKEN_SYMBOL && userCollateral.amount > 0.002*std::pow(10.0, PETH_TOKEN_SYMBOL.precision()))
      memo = "swap,1,765-11";
    else if (userCollateral.symbol == BOX_TOKEN_SYMBOL && userCollateral.amount > 0.3*std::pow(10.0, BOX_TOKEN_SYMBOL.precision()))
      memo = "swap,1,194-11";
    else if (symbolIterator->contract == eosio::name("lptoken.defi") && userCollateral.amount > 5000) // LP tokens, send these to swap.defi, it sends back both swap tokens with memo Defibox: withdraw
      memo = "";
    else
        continue;
...
if(count>0) payminer(miner, _config.getMinerFee());
}
...
void vigorlending::rewardclaim(const eosio::name& miner, eosio::name contract){
...
payminer(miner, _config.getMinerFee());
}

void vigorlending::payminer(const eosio::name& miner, uint64_t fee){
      ...
      if(vitr != _u.end()) {
      if (vitr->getCollaterals().hasAsset(gasfee,vig) && vig.amount >= gasfee.amount) {
...
void vigorlending::assetin(const eosio::name& from, const eosio::name& to, const eosio::asset& assetin, const std::string& memo) {
  ...
  if((from == _config.getProxyPay() && assetin.symbol.raw() == eosio::symbol("GEN", 8).raw() && contract.value == eosio::name("genpooltoken").value) ||
    ((from == eosio::name("swap.defi") && memo == "Defibox: withdraw" && // tokens not on whitelist inbound from redeeming LP tokens
  ...
  eosio::asset localAssetIn = asset_swap(assetin);
  auto userIterator = _u.find(from.value);
  auto vitr = _u.require_find(_config.getVigorDacFund().value, "VigorDacFund not found");

  if(from == _config.getProxyPay() && localAssetIn.symbol == SYSTEM_TOKEN_SYMBOL){
    userIterator = vitr;
    _g.rexproxy += localAssetIn.amount; //accumulate proxy vote rewards
    _u.modify(vitr, get_self(), [&](auto& modified_user) {
  ...
  if(from == ZAP_CONTRACT){
    if (assetin.symbol == VIGOR_SYMBOL) { //excess VIGOR from zap
    eosio::asset payoff = eosio::asset(std::min(localAssetIn.amount, vitr->debt.amount), VIGOR_SYMBOL); // payoff VIGOR debt, up to the amount vigordacfund has, deposit remainder to l_debt
    eosio::asset remainder = localAssetIn.amount > vitr->debt.amount ? eosio::asset(localAssetIn.amount - vitr->debt.amount, VIGOR_SYMBOL) : eosio::asset(0, VIGOR_SYMBOL);
    if (payoff.amount > 0)
    _u.modify(vitr, get_self(), [&](auto& modified_user) {
     modified_user.debt -= payoff;
     _g.totaldebt -= payoff;
#ifdef TEST_MODE
#else
    //clear the debt from circulating supply
    ...
    eosio::action(eosio::permission_level {get_self(), "active"_n},
                  symbolIterator->contract,
                  "retire"_n,
                  std::make_tuple(payoff, std::string("vigordacfund payoff VIGOR debt")))
      .send();
#endif
    });
    if (remainder.amount > 0) {
  ...
  if(assetin.symbol == SXE_TOKEN_SYMBOL && from != ZAP_CONTRACT)
    eosio::check(false, "Deposit of SXE is disabled");
  
  if(from == eosio::name("swap.defi") || from == eosio::name("mine2.defi")){ //proceeds from miners swapping tokens for VIG, some usage of the swaps delivers mining rewards from mine2.defi
    _u.modify(vitr, get_self(), [&](auto& modified_user) {
        getCollaterals(modified_user) += localAssetIn;
    });
    update(*vitr);
...
// rewards handler
  if(from == eosio::name("lptoken.defi") || from == eosio::name("lppool.defi") || from == eosio::name("usnpool.defi")) {
    if(assetin.symbol.raw() == SYSTEM_TOKEN_SYMBOL.raw())
    ...
    else // other tokens which are not whitelisted being received as rewards
      return;
    _u.modify(vitr, get_self(), [&](auto& modified_user) {
        getCollaterals(modified_user) += localAssetIn;
    });
  ...
  if(from != ZAP_CONTRACT && from != _config.getProxyPay()) {
  ...
  auto market = _market.get(symbol_swap(localAssetIn.symbol).raw());
  double valueofassetin = (localAssetIn.amount) / std::pow(10.0, localAssetIn.symbol.precision()) * ((double)market.marketdata.price.back() / pricePrecision);
  if(from != _config.getProxyPay() && from != ZAP_CONTRACT) {
    //eosio::asset fee = asset_swap(eosio::asset(std::max((uint64_t)(std::pow(10.0, symbol_swap(localAssetIn.symbol).precision()) * (std::min(std::max(valueofassetin * _config.getTxnFee() / 10000.0, 0.0), 10.0) / ((double)market.marketdata.price.back() / pricePrecision))),(uint64_t)1),symbol_swap(localAssetIn.symbol)));
    //fee.amount = ((localAssetIn.amount - fee.amount) <= 0 || memo == "payback borrowed token") ? 0 : fee.amount;
    //localAssetIn -= fee;
    //takefee(fee);
    //_g.txnfee += fee.amount / std::pow(10.0, fee.symbol.precision()) * ((double)market.marketdata.price.back() / pricePrecision);
  }
  ...
  } else if (from != _config.getProxyPay() && from != ZAP_CONTRACT){
      if (symbol_swap(localAssetIn.symbol) == USDT_SYMBOL){ // all USDT deposits are zapped into SXE
        // query amount of liquid USDT in vigorlending
        eosio::asset usdtbalance = eosio::asset(0,USDT_SYMBOL);
        auto symbolIterator = _whitelist.find(symbol_swap(localAssetIn.symbol).raw());
        if(symbolIterator != _whitelist.end()) {
          accounts accounts_table(symbolIterator->contract, get_self().value);
          const auto& itr = accounts_table.find(symbol_swap(localAssetIn.symbol).code().raw());
          if(itr != accounts_table.end())
            usdtbalance = itr->balance;
        }
        ...
        if (itr != curvesx_pairs.end()) {
          auto symbolIterator = _whitelist.find(symbol_swap(localAssetIn.symbol).raw());
          eosio::action(eosio::permission_level {get_self(), eosio::name("active")},
                        symbolIterator->contract,
                        eosio::name("transfer"),
                        std::make_tuple(get_self(), ZAP_CONTRACT, usdtbalance, std::string("SXE")))
          .send();
          double vol_last = _g.curvesxvol;
          _g.curvesxvol = (itr->volume0.amount / std::pow(10.0, USDT_SYMBOL.precision())) * ((double)_market.get(USDT_SYMBOL.raw()).marketdata.price.back() / pricePrecision) + 
                          (itr->volume1.amount / std::pow(10.0, VIGOR_SYMBOL.precision())) * ((double)_market.get(VIGOR_SYMBOL.raw()).marketdata.price.back() / pricePrecision);
          double tvl = (itr->reserve0.quantity.amount / std::pow(10.0, USDT_SYMBOL.precision())) * ((double)_market.get(USDT_SYMBOL.raw()).marketdata.price.back() / pricePrecision) + 
                       (itr->reserve1.quantity.amount / std::pow(10.0, VIGOR_SYMBOL.precision())) * ((double)_market.get(VIGOR_SYMBOL.raw()).marketdata.price.back() / pricePrecision);
          curvesx::config_table ct(CURVESX_CONTRACT, CURVESX_CONTRACT.value);
          auto curvesx_config = ct.get();
          _g.curvesxrew += ((curvesx_config.protocol_fee + curvesx_config.trade_fee)/10000.0) * (itr->virtual_price / tvl) * (_g.curvesxvol - vol_last); // accumulate fees gained from curve.sx trading activity
        }
      }
      _u.modify(userIterator, get_self(), [&](auto& modified_user) {
        if(memo == "insurance") {
...
void vigorlending::takefee(eosio::asset fee) {
  if(fee.amount == 0)
    return;
  auto vigordacfund = _u.require_find(_config.getVigorDacFund().value, "VigorDacFund not found");
  if (fee.symbol.code() != VIGOR_SYMBOL.code()){
    _u.modify(vigordacfund, get_self(), [&](auto& modified_user) {
      getCollaterals(modified_user) += fee; // add fee to vigordacfund and global collateral
    });
  } else if (fee.symbol.code() == VIGOR_SYMBOL.code()) {
    eosio::asset payoff = eosio::asset(std::min(fee.amount, vigordacfund->debt.amount), VIGOR_SYMBOL); // payoff VIGOR debt, up to the amount vigordacfund has
    eosio::asset remainder = fee.amount > vigordacfund->debt.amount ? eosio::asset(fee.amount - vigordacfund->debt.amount, VIGOR_SYMBOL) : eosio::asset(0, VIGOR_SYMBOL);
    if (payoff.amount > 0)
    _u.modify(vigordacfund, get_self(), [&](auto& modified_user) {
     modified_user.debt -= payoff;
     _g.totaldebt -= payoff;
...
void vigorlending::assetout(const eosio::name& usern, const eosio::asset& assetout, const std::string& memo) {
  ...
  eosio::check(memo == "collateral" ||
                 memo == "insurance" ||
                 memo == "borrow" ||
                 memo == "cas" || // synthetics create and stake
                 memo == "stakerefund" ||
                 memo == "savings" ||
                 memo == "vigfees",
               "memo must be composed of either word: 'insurance' | 'collateral' | 'borrow' | 'stakerefund' | 'savings' | 'vigfees' | 'cas'");
  if(assetout.symbol == SXE_TOKEN_SYMBOL)
    eosio::check(memo == "collateral" || memo == "insurance", "memo must be composed of either word: 'insurance' | 'collateral");
...
void vigorlending::tick(const eosio::name& miner) {
  ...
  if(_g.step != 0) {
    eosio::action(
      ...
      std::make_tuple())
      .send();
      payminer(miner, _config.getTickFee());
  } else {
    croneos::queue _queue(get_self());
...
void vigorlending::exassetout(const eosio::name& usern, const eosio::asset& assetout, const std::string& memo, const bool& exec, const bool partialupdate, const uint64_t& step, double& totalval, double& totalvalbefore) {
  ...
  eosio::asset localAssetOut = asset_swap(assetout);
  ...
  if(memo == "borrow" && assetout.symbol == VIGOR_SYMBOL) {
    ...
    double valueofcol = user.valueofcol;
    double W1 = 0.0;
    double fee = std::min(std::max(valueofasset * _config.getTxnFee() / 10000.0, 0.0), 10.0);
    auto vigordacfund = _u.require_find(_config.getVigorDacFund().value, "VigorDacFund not found");
    double pct = valueofcol > 0.0 ? std::min(fee / valueofcol, 1.0) : 0.0;
    if (pct > 0.0) {
    auto userCollaterals = getCollaterals(const_cast<user_s&>(user));
    ...
    eosio::check(valueofcol >= mincol * 0.998 * ((user.debt.amount + localAssetOut.amount) / std::pow(10.0, localAssetOut.symbol.precision())),
                 "Max allowable borrow " + eosio::asset(std::pow(10.0, localAssetOut.symbol.precision()) * std::max((valueofcol / mincol) - (user.debt.amount / std::pow(10.0, localAssetOut.symbol.precision())), 0.0), localAssetOut.symbol).to_string() +
...
#ifdef TEST_MODE
      if(_g.totaldebt.amount + localAssetOut.amount > _config.getDebtCeiling() * std::pow(10.0, VIGOR_SYMBOL.precision()))
#else
      if(st.supply.amount + localAssetOut.amount > _config.getDebtCeiling() * std::pow(10.0, VIGOR_SYMBOL.precision()))
#endif
        debtCeiling = true;
    }
    ...
    if(exec) {
      ...
      _u.modify(user, get_self(), [&](auto& modified_user) {
        modified_user.debt += localAssetOut;
      });
    ...
    if(memo == "insurance") {
      ...
      valueofasset = localAssetOut.amount / std::pow(10.0, insurance.symbol.precision()) * (double)market.marketdata.price.back() / pricePrecision;
      eosio::asset fee = asset_swap(eosio::asset(std::max((uint64_t)(std::pow(10.0, symbol_swap(localAssetOut.symbol).precision()) * (std::min(std::max(valueofasset * _config.getTxnFee() / 10000.0, 0.0), 10.0) / ((double)market.marketdata.price.back() / pricePrecision))),(uint64_t)1),symbol_swap(localAssetOut.symbol)));
      fee.amount = (localAssetOut.amount - fee.amount) <= 0 ? 0 : fee.amount;
      ...
      if(exec) {
        ...
        takefee(fee);
        _g.txnfee += fee.amount / std::pow(10.0, localAssetOut.symbol.precision()) * ((double)market.marketdata.price.back() / pricePrecision);
        localAssetOut -= fee;
        if (symbol_swap(localAssetOut.symbol) == USDT_SYMBOL){ // all USDT withdrawls are zapped out of SXE
          // query amount of liquid USDT in vigorlending
          eosio::asset usdtbalance = eosio::asset(0,USDT_SYMBOL);
            auto symbolIterator = _whitelist.find(symbol_swap(localAssetOut.symbol).raw());
            if(symbolIterator != _whitelist.end()) {
              accounts accounts_table(symbolIterator->contract, get_self().value);
              const auto& itr = accounts_table.find(symbol_swap(localAssetOut.symbol).code().raw());
              if(itr != accounts_table.end())
                usdtbalance = itr->balance;
            }
          if (usdtbalance < localAssetOut) { // if need to liquidate SXE to get USDT
            curvesx::pairs_table curvesx_pairs(CURVESX_CONTRACT, CURVESX_CONTRACT.value);
            auto itr = curvesx_pairs.find(eosio::symbol("SXE",4).code().raw()); 
            if (itr != curvesx_pairs.end()) {
                eosio::asset SXEtozap = eosio::asset(std::max(1.02*asset_swap(localAssetOut).amount / ((itr->reserve0.quantity.amount / (itr->liquidity.quantity.amount * itr->virtual_price))+((itr->reserve1.quantity.amount * itr->price1_last) / (itr->liquidity.quantity.amount * itr->virtual_price))),5*std::pow(10.0, 4)), eosio::symbol("SXE",4));
                eosio::action(eosio::permission_level {get_self(), eosio::name("active")},
                              CURVELP_TOKEN_CONTRACT,
                              eosio::name("transfer"),
                              std::make_tuple(get_self(), ZAP_CONTRACT, SXEtozap, std::string("USDT")))
                .send();
            double vol_last = _g.curvesxvol;
            _g.curvesxvol = (itr->volume0.amount / std::pow(10.0, USDT_SYMBOL.precision())) * ((double)_market.get(USDT_SYMBOL.raw()).marketdata.price.back() / pricePrecision) + 
                            (itr->volume1.amount / std::pow(10.0, VIGOR_SYMBOL.precision())) * ((double)_market.get(VIGOR_SYMBOL.raw()).marketdata.price.back() / pricePrecision);
        ...
        std::string message = user.usern.to_string() + " withdraw tokens from lending/insurance pool " + (localAssetOut + fee).to_string();
        tracetable(__func__, __LINE__, userntrace, bailoutid, message);
      }
    }
    if(memo == "savings") {
      ...
      auto market = _market.get(symbol_swap(localAssetOut.symbol).raw());
      valueofasset = localAssetOut.amount / std::pow(10.0, collateral.symbol.precision()) * (double)market.marketdata.price.back() / pricePrecision;
      eosio::asset fee = asset_swap(eosio::asset(std::max((uint64_t)(std::pow(10.0, symbol_swap(localAssetOut.symbol).precision()) * (std::min(std::max(valueofasset * _config.getTxnFee() / 10000.0, 0.0), 10.0) / ((double)market.marketdata.price.back() / pricePrecision))),(uint64_t)1),symbol_swap(localAssetOut.symbol)));
      fee.amount = (localAssetOut.amount - fee.amount) <= 0 ? 0 : fee.amount;
      ...
      if(exec) {
        ...
        localAssetOut -= fee;
        if (symbol_swap(localAssetOut.symbol) == USDT_SYMBOL){ // all USDT withdrawls are zapped out of SXE
          //eosio::check(asset_swap(localAssetOut).amount >= 5.0 * std::pow(10.0, asset_swap(localAssetOut).symbol.precision()), "quantity too small, minimum size 5 USDT");
          // query amount of liquid USDT in vigorlending
          eosio::asset usdtbalance = eosio::asset(0,USDT_SYMBOL);
            auto symbolIterator = _whitelist.find(symbol_swap(localAssetOut.symbol).raw());
            if(symbolIterator != _whitelist.end()) {
        ...
        std::string message = user.usern.to_string() + " withdraw tokens from collateral pool " + (localAssetOut + fee).to_string();
        tracetable(__func__, __LINE__, userntrace, bailoutid, message);
      }
      ...
      eosio::check(symbol_swap(localAssetOut.symbol).raw() != USDT_SYMBOL.raw(), "Borrwing USDT is not allowed.");

      auto market = _market.get(symbol_swap(localAssetOut.symbol).raw());
      valueofasset = (localAssetOut.amount) / std::pow(10.0, localAssetOut.symbol.precision()) * ((double)market.marketdata.price.back() / pricePrecision);
      eosio::asset fee = eosio::asset(std::max((uint64_t)(std::pow(10.0, localAssetOut.symbol.precision()) * (std::min(std::max(valueofasset * _config.getTxnFee() / 10000.0, 0.0), 10.0) / ((double)market.marketdata.price.back() / pricePrecision))),(uint64_t)1),localAssetOut.symbol);
      localAssetOut += fee;
      valueofasset = (localAssetOut.amount) / std::pow(10.0, localAssetOut.symbol.precision()) * ((double)market.marketdata.price.back() / pricePrecision);

      if(!(user.l_valueofcol + valueofasset <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())) / (mincolup * 0.998))) {
      ...
      // borrow synthetics against VIGOR as collateral
      ...
      valueofasset = (localAssetOut.amount / std::pow(10.0, localAssetOut.symbol.precision()));
      eosio::asset fee = eosio::asset(std::max((uint64_t)(std::pow(10.0, localAssetOut.symbol.precision()) * (std::min(std::max(valueofasset * _config.getTxnFee() / 10000.0, 0.0), 10.0))),(uint64_t)1),localAssetOut.symbol);

      eosio::check(user.l_valueofcol * mincolup * 0.998 <= ((user.l_debt.amount - localAssetOut.amount) / std::pow(10.0, localAssetOut.symbol.precision())),
                   "Max allowable collateral withdraw " + eosio::asset(std::pow(10.0, localAssetOut.symbol.precision()) * std::max(((user.l_debt.amount / std::pow(10.0, localAssetOut.symbol.precision())) - (user.l_valueofcol * mincolup)), 0.0), localAssetOut.symbol).to_string() +
                     ". Collateral Ratio limit " + std::to_string(mincolup));
      ...
      if(exec) {
        _g.volume += valueofasset;
        _u.modify(userIterator, get_self(), [&](auto& modified_user) {
          modified_user.l_debt -= localAssetOut;
          modified_user.txnvolume += valueofasset;
        });
        ...
        takefee(fee);
        _g.txnfee += (fee.amount / std::pow(10.0, localAssetOut.symbol.precision()));
        localAssetOut -= fee;

        std::string message = user.usern.to_string() + " withdraw VIGOR from l_debt " + (localAssetOut + fee).to_string();
        tracetable(__func__, __LINE__, userntrace, bailoutid, message);
      }
...
void vigorlending::distribfee() {
  ...
  double lptokensxamt = (_g.txnfee * _config.getLpsxCut() + _g.curvesxrew); // cut of transaction fees from both vigorlending and curve.sx, in dollars for deposits of USDT
  ...
  for(const auto& insurance : *user.getInsurances()) {
    ...
    if (symbolIterator->sym.raw() == USDT_SYMBOL.raw())
  ...
  lptokensx = _g.totrewlendsx == 0.0 ? 0.0 : lptokensx / _g.totrewlendsx;
    ...
    _u.modify(user, get_self(), [&](auto& modified_user) {
      ...
      modified_user.rewardlend += (rewardlend/std::pow(10.0, _g.fee.symbol.precision()))*vigprice;
      double days_since_anniv = (3600.0 + eosio::current_time_point().sec_since_epoch() - (modified_user.getAnniv().sec_since_epoch() - _config.getRepAnniv())) / eosio::days(1).to_seconds();
  ...
  if(itr == _u.end()) {
    ...
    _g.lprewards = 0.0;
    _g.curvesxrewa += _g.curvesxrew; // accumulate until atimer resets to zero
    _g.curvesxrew = 0.0;
...
void vigorlending::performanceglobal() {
  ...
  double rewards = 365.0 * (rexproxy + _g.lprewardsp + (_g.txnfeep * _config.getLpsxCut() + _g.curvesxrewp)); // dollars worth of rewards dsitribed to this user (REX/vote and LP token rewards, curve.sx and vigorlending txn fees) for one period (atimer peiod), then scaled to annual
...
void vigorlending::recapup(const user_s& user, const user_s& insurerUser, double insurerlpcts, double& sumpcts, bool lastInsurer) {
  ...
  if(existing != _currency_statst.end()) {
    const auto& st = *existing;
#ifdef TEST_MODE
    if(_g.totaldebt.amount  + vigorborrowa.amount > _config.getDebtCeiling() * std::pow(10.0, VIGOR_SYMBOL.precision()))
#else
