...
void vigorlending::doupdate() {
  ...
  switch(_g.step) {
    ...
    case 17: {
      ...
      double vigprice = market.marketdata.price[0] / pricePrecision;
      ...
      if(itr == _u.end()) {
        ...
        _g.availability.erase(std::remove_if(_g.availability.begin(), _g.availability.end(),
                              ...
                              _g.availability.end());
        _g.totalvalue = _g.valueofcol + _g.valueofins + _g.savings * ((double)_market.get(VIGOR_SYMBOL.raw()).marketdata.price[0] / pricePrecision) + (_g.l_totaldebt.amount / std::pow(10.0, _g.l_totaldebt.symbol.precision())) - _g.l_valueofcol - (_g.totaldebt.amount / std::pow(10.0, _g.totaldebt.symbol.precision()));
        _g.volume = 0.0;
        _g.step = 0;
...
void vigorlending::schaction(const eosio::name& tag, const eosio::name& action_name, const std::tuple<T...> data, const uint32_t& delay_sec, const uint32_t& expiry_sec, const uint64_t& repeat) {
  ...
  _queue.schedule(
    eosio::action(eosio::permission_level {get_self(), eosio::name("active")},
                  ...
                  move(data)),
    tag,
    eosio::asset(_config.getGasFee(), VIG4_SYMBOL),
    delay_sec,
    expiry_sec,
    repeat);
}
...
void vigorlending::kick(const eosio::name& usern) {
  // /METHOD_CONTEXT;
  ...
  schaction(get_self(), eosio::name("returnins"), std::make_tuple(usern), 0, _config.getMineBuffer() * 2, 1);
  schaction(get_self(), eosio::name("bailout"), std::make_tuple(usern), 0, _config.getMineBuffer() * 2, 1);
  schaction(get_self(), eosio::name("bailoutup"), std::make_tuple(usern), 0, _config.getMineBuffer() * 3, 1);
  schaction(get_self(), eosio::name("returncol"), std::make_tuple(usern), 0, _config.getMineBuffer() * 3, 1);
  schaction(get_self(), eosio::name("dodeleteacnt"), std::make_tuple(usern), 0, _config.getMineBuffer() * 5, 1);
...
void vigorlending::liquidate(const eosio::name& usern) {
  ...
  eosio::check(_g.step == 0, "Contract is updating, try again later.");
...
// self liquidation for crypto debt (not VIGOR), cost is a reduction of credit score
// user can quickly exit a loan
// user gives debt to insuers/lenders along with an equivalent value of collateral (user retaining excess collateral)
...
void vigorlending::openaccount(const eosio::name& owner) {
  ...
  if(owner.value == _config.getProxyPay().value)
    eosio::check(false, "This account cannot be a user, it is a proxy reward payer: " + _config.getProxyPay().to_string());
...
void vigorlending::dodeleteacnt(const eosio::name& owner) {
  ...
  eosio::asset dust;
  if(user.getInsurances().hasAsset(_g.fee, dust)) {
    if(dust.amount < 1000000 && dust.amount > 0) {
      _u.modify(user, get_self(), [&](auto& modified_user) {
        modified_user.getInsurances() -= dust;
      });
      auto& finalreserve = _u.get(_config.getFinalReserve().value, "finalreserve not found");
      _u.modify(finalreserve, get_self(), [&](auto& modified_user) {
        modified_user.getInsurances() += dust;
      });
    }
  }
  if(user.getLCollaterals().hasAsset(_g.fee, dust)) {
    if(dust.amount < 1000000 && dust.amount > 0) {
      _u.modify(user, get_self(), [&](auto& modified_user) {
  ...
  if(user.getCollaterals().hasAsset(_g.fee, dust)) {
    if(dust.amount < 1000000 && dust.amount > 0) {
      _u.modify(user, get_self(), [&](auto& modified_user) {
  ...
  dust = eosio::asset(user.vigfees, VIG10_SYMBOL);
  if(dust.amount < 1000000 && dust.amount > 0) {
    _u.modify(user, get_self(), [&](auto& modified_user) {
  ...
  if(dustd < 0.0001 && dustd > 0.0) {
    ...
    auto& vigordacfund = _u.get(_config.getVigorDacFund().value, "VigorDacFund not found");
...
void vigorlending::collectfee() {
  ...
  auto market = _market.get(_g.fee.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : _g.fee.symbol.raw());
  double vigprice = market.marketdata.price[0] / pricePrecision;
  double vigvol = market.marketdata.vol / volPrecision;
  if(_b.usern.value == _u.begin()->usern.value) {
  ...
  while(itr != stop_itr && counter++ < _b.batchsize) {
    ...
    // calculate token swap payment due, denominated in dollars
    double tespay = (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) * (std::pow((1.0 + user.tesprice), T) - 1.0);
    double l_tespay = user.l_valueofcol * (std::pow((1.0 + user.l_tesprice), T) - 1.0);

    // convert to number of VIG
    eosio::asset amta = eosio::asset((tespay * std::pow(10.0, VIG10_SYMBOL.precision())) / vigprice, VIG10_SYMBOL);
    eosio::asset l_amta = eosio::asset((l_tespay * std::pow(10.0, VIG10_SYMBOL.precision())) / vigprice, VIG10_SYMBOL);

    eosio::asset vigfees = eosio::asset(user.vigfees, VIG10_SYMBOL);
    if((amta.amount + l_amta.amount > vigfees.amount) || (uint64_t)(vigfees.amount / std::pow(10.0, 6)) <= 0) {
      //insufficient VIG for paying fees, send user to viglifeline to obtain VIG into VIG fees, approximately enough to last for _config.getvigLifeLine() days.
      tespay = (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) * (std::pow(1.0 + (user.tesprice == 0.0 ? _config.getMaxTesPrice() / 2.0 : user.tesprice), _config.getvigLifeLine() / 360.0) - 1.0);
      l_tespay = user.l_valueofcol * (std::pow(1.0 + (user.l_tesprice == 0.0 ? _config.getMaxTesPrice() / 2.0 : user.l_tesprice), _config.getvigLifeLine() / 360.0) - 1.0);
      eosio::asset tespayvigor = eosio::asset(std::max((int64_t)(tespay * std::pow(10.0, VIGOR_SYMBOL.precision())), (int64_t)1), VIGOR_SYMBOL);
      eosio::asset l_tespayvigor = eosio::asset(std::max((int64_t)(l_tespay * std::pow(10.0, VIGOR_SYMBOL.precision())), (int64_t)1), VIGOR_SYMBOL);
      eosio::asset vigneededa = eosio::asset(uint64_t(((tespayvigor.amount / std::pow(10.0, VIGOR_SYMBOL.precision())) / vigprice) * std::pow(10.0, VIG10_SYMBOL.precision())), VIG10_SYMBOL);
      vigneededa.amount += uint64_t(((l_tespayvigor.amount / std::pow(10.0, VIGOR_SYMBOL.precision())) / vigprice) * std::pow(10.0, VIG10_SYMBOL.precision()));
      viglifeline(user, vigfees, vigneededa, vigprice, vigvol);
    }

    _u.modify(user, get_self(), [&](auto& modified_user) {
      if(amta.amount + l_amta.amount > vigfees.amount) {
        // PARTIAL_PAYMENTS
        modified_user.prem += vigfees.amount / std::pow(10.0, vigfees.symbol.precision());
        _g.fee.amount += vigfees.amount;
        if(user.debt.amount > 0 && _g.bailoutuser == eosio::name(0))
          _g.bailoutuser = user.usern;
        if(user.l_valueofcol > 0.0 && _g.bailoutupuser == eosio::name(0))
          _g.bailoutupuser = user.usern;
        modified_user.vigfees -= vigfees.amount;
        _g.vigfees -= vigfees.amount;  //subtract partial fee from user vigfees and global vigfees
        ...
        modified_user.vigfees -= (amta.amount + l_amta.amount);
        _g.vigfees -= (amta.amount + l_amta.amount);  //subtract fee from user vigfees and global vigfees
      }
    });
    itr++;
  }
...
void vigorlending::viglifeline(const user_s& user, eosio::asset& vigfees, eosio::asset vigneededa, double vigprice, double vigvol) {
  const eosio::name& userntrace = user.usern;
  const uint64_t bailoutid = 0;
  // not enough VIG so borrow enough to last vigLifeLine number of days
  double vigneeded = vigneededa.amount / std::pow(10.0, vigneededa.symbol.precision()) * vigprice;
  double overcol = 1.0 + 2.0 * vigvol / sqrt(12);  // overcollateralize the VIG borrow by this amount, enough collateral to survive a 2 * monthly standard deviation move of VIG price
  std::string message = "VIG lifeline: " + user.usern.to_string() + " VIG lifeline initiated for " + vigneededa.to_string() + " to last approximately " + std::to_string(_config.getvigLifeLine()) + " days for paying fees. ";
  eosio::asset c;
  eosio::asset d;
  eosio::asset globalInsurance;
  eosio::asset vigorneededa = eosio::asset((int64_t)(overcol * vigneeded * std::pow(10.0, VIGOR_SYMBOL.precision())), VIGOR_SYMBOL);
  if(user.getInsurances().hasAsset(_g.fee, c) && c.amount >= vigneededa.amount && _g.getInsurances().hasAsset(_g.fee, globalInsurance) && globalInsurance.amount >= vigneededa.amount) {
    message.append(std::string("Success. Moved VIG from user insurance/lending to vigfees: " + vigneededa.to_string()));
    tracetable(__func__, __LINE__, userntrace, bailoutid, message);
    _u.modify(user, get_self(), [&](auto& modified_user) {
      modified_user.vigfees += vigneededa.amount;
      _g.vigfees += vigneededa.amount;
      getInsurances(modified_user) -= vigneededa;
    });
    vigfees.amount += vigneededa.amount;
  } else if(user.getCollaterals().hasAsset(_g.fee, d) && d.amount >= vigneededa.amount && (user.valueofcol - vigneeded >= 1.02 * (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())))) {
    message.append(std::string("Success. Moved VIG from user collateral to vigfees: " + vigneededa.to_string()));
    tracetable(__func__, __LINE__, userntrace, bailoutid, message);
    _u.modify(user, get_self(), [&](auto& modified_user) {
      modified_user.vigfees += vigneededa.amount;
      _g.vigfees += vigneededa.amount;
      getCollaterals(modified_user) -= vigneededa;
    });
    vigfees.amount += vigneededa.amount;
  } else {
    ...
    if(existing != _currency_statst.end()) {
      ...
      if(st.supply.amount + vigorneededa.amount > _config.getDebtCeiling() * std::pow(10.0, vigorneededa.symbol.precision()))
        debtCeiling = true;
    }
    if(!debtCeiling) {
      double mincol = 1.0;
      if((((user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) + overcol * vigneeded) * mincol <= user.valueofcol) ||
         ((user.l_valueofcol + vigneeded) * mincol <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())))) {
        // borrow VIGOR debt against collateral and move it to l_debt, then borrow VIG against l_debt
        ...
        eosio::asset lr = eosio::asset(0, vigneededa.symbol);
        if(finalreserve.getInsurances().hasAsset(vigneededa, reserveAsset))
        ...
        if(globalInsurances.hasAsset(vigneededa, globalInsuranceAsset)) {
          if(globalInsuranceAsset.amount >= vigneededa.amount) {
            int64_t maxBorrowAmount = (globalInsuranceAsset - lr).amount - vigneededa.amount;
            if(maxBorrowAmount >= 0) {
              if(!((user.l_valueofcol + vigneeded) * mincol <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision()))) ||
                 ((user.valueofcol - ((user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) + overcol * vigneeded) * mincol) >
                  ((user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())) - (user.l_valueofcol + vigneeded) * mincol))) {
                // two conditions when viglifeline will borrow VIGOR as debt against collateral, and pass that VIGOR into l_debt
                // 1) if user does not have enough VIGOR in l_debt to borrow VIG as l_collateral
                // 2) user has enough VIGOR in l_debt already to borrow VIG as l_collateral, but user has more capacity to borrow VIGOR as debt against collateral, than to use their capacity of l_debt.
                //    the result is that viglifeline will switch between the two sources of VIGOR as not to deplete one source completely to zero in spite of the other
                message.append(std::string("Success. Borrowed " + vigorneededa.to_string() + " to use as collateral to borrow VIG. "));
                //borrow VIGOR debt against collateral and move it to l_debt
                _u.modify(_u.iterator_to(user), get_self(), [&](auto& modified_user) {
                  modified_user.debt += vigorneededa;
                  modified_user.l_debt += vigorneededa;
                });
                _g.totaldebt += vigorneededa;
                _g.l_totaldebt += vigorneededa;

                auto symbolIterator = _whitelist.find(VIGOR_SYMBOL.raw());
                eosio::action(eosio::permission_level {get_self(), eosio::name("active")},
                              symbolIterator->contract,  // contract for vigor token
                              eosio::name("issue"),
                              std::make_tuple(get_self(), vigorneededa, std::string("VIGOR issued to ") + get_self().to_string()))
                  .send();
              }

              // borrow VIG against l_debt, and put it into user vigfees
              // add the located asset to the borrower vigfees, borrower l_collateral, and global vigfees
              message.append(std::string("Success. Borrowed " + vigneededa.to_string() + ", sent to vigfees. "));
              //message.append(std::string( "add the located asset to the borrower vigfees, borrower l_collateral, and global vigfees " + vigneededa.to_string() ));
              _u.modify(user, get_self(), [&](auto& modified_user) {
                modified_user.vigfees += vigneededa.amount;
                _g.vigfees += vigneededa.amount;
                modified_user.getLCollaterals() += vigneededa;
                vigfees.amount += vigneededa.amount;
              });

              // subtract the located asset from the global insurance and add to global l_collateral
              //message.append(std::string( "subtract the located asset from the global insurance and add to global l_collateral" + vigneededa.to_string() ));
              globalInsurances.move(vigneededa, _g.getLCollaterals());
              // don't subtract the located asset from any particular insurer, just subtract from global insurance
            } else {
              eosio::asset remavail = vigneededa.symbol == VIG10_SYMBOL ? eosio::asset((vigneededa.amount + maxBorrowAmount) / std::pow(10.0, 6), VIG4_SYMBOL) : eosio::asset(vigneededa.amount + maxBorrowAmount, vigneededa.symbol);
              std::string remavailm = std::string("Fail. Can't locate enough to borrow. Available to borrow from lending pool: " + remavail.to_string());
              message.append(remavailm);
            }
          } else {
            message.append(std::string("Fail. Can't borrow VIG, insufficient lending assets available in global."));
          }
        } else {
          message.append(std::string("Fail. Can't borrow VIG, Token not available for borrowing."));
        }

        tracetable(__func__, __LINE__, userntrace, bailoutid, message);
      } else {
        message.append(std::string("Fail. Can't borrow VIG, user does not have enough collateral."));
        tracetable(__func__, __LINE__, userntrace, bailoutid, message);
      }
    } else {
      message.append(std::string("Fail. Can't borrow VIG. The VIGOR debt ceiling has been reached."));
      tracetable(__func__, __LINE__, userntrace, bailoutid, message);
    }
  }
}
...
void vigorlending::assetin(const eosio::name& from, const eosio::name& to, const eosio::asset& assetin, const std::string& memo) {
  ...
  eosio::name contract = get_first_receiver();
  ...
  if(from == _config.getProxyPay() && assetin.symbol == SYSTEM_TOKEN_SYMBOL) {
    if(_config.getRexSwitch() >= 1) {
      ...
      eosio::action(eosio::permission_level {get_self(), "active"_n},
                    eosio::name("eosio"),
                    eosio::name("deposit"),
                    std::make_tuple(get_self(), assetin))
        .send();
      eosio::action(eosio::permission_level {get_self(), "active"_n},
                    eosio::name("eosio"),
                    eosio::name("buyrex"),
  ...
  const eosio::asset& localAssetIn = assetin.symbol == VIG4_SYMBOL ? eosio::asset(assetin.amount * std::pow(10.0, 6), VIG10_SYMBOL) : assetin;
  ...
  if(userIterator == _u.end() && memo == "stake") {
    ...
    _staked.modify(stakeduser, get_self(), [&](auto& st) {
      auto market = _market.get(localAssetIn.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : localAssetIn.symbol.raw());
      uint64_t reqstake = (std::pow(10.0, localAssetIn.symbol.precision()) * _config.getReqStake()) / ((double)market.marketdata.price[0] / pricePrecision);
      ...
      st.unlocktime = eosio::time_point_sec(eosio::current_time_point()) + _config.getStakeTime();
    });
    return;
  }
  ...
  if(_market.begin() == _market.end()) {
    for(const auto& w : _whitelist) {
      ...
      if(mitr != _market.end()) {
        if(stats.begin()->price[0] != mitr->marketdata.price[0] ||
  ...
  double totalvalbefore = _g.valueofcol + _g.valueofins + _g.savings * ((double)_market.get(VIGOR_SYMBOL.raw()).marketdata.price[0] / pricePrecision) - _g.l_valueofcol - (_g.totaldebt.amount / std::pow(10.0, _g.totaldebt.symbol.precision()));
  for(const auto& w : _whitelist) {
    t_series stats(_config.getOracleHub(), w.feed.value);
    auto mitr = _market.find(w.sym.raw());
    if(mitr != _market.end()) {
  ...
  double valueofassetin = (localAssetIn.amount) / std::pow(10.0, localAssetIn.symbol.precision()) *
                          ((double)market.marketdata.price[0] / pricePrecision);
  ...
  update(user);
  double vigorprice = ((double)_market.get(VIGOR_SYMBOL.raw()).marketdata.price[0] / pricePrecision);
...
void vigorlending::paybacktok(const user_s& user, eosio::asset assetin) {
  eosio::print("paybacktok: " + assetin.to_string(), "\n");
  // assetin
  // ass
  ///METHOD_CONTEXT;
  const eosio::name& userntrace = user.usern;
  ...
  eosio::check(l_collateral >= assetin, "Payback amount too high. " + l_collateral.to_string() + " >= " + assetin.to_string() + " owner: " + userntrace.to_string());

  if(assetin.symbol == VIG10_SYMBOL && (l_collateral.amount - assetin.amount) < 1000000 && (l_collateral.amount - assetin.amount) > 0) {
    eosio::asset dust = l_collateral - assetin;
    ///tracetable(__func__, __LINE__, user.usern, " gives dust to the finalreserve l_collateral: ", dust);
    _u.modify(user, get_self(), [&](auto& modified_user) {
      modified_user.getLCollaterals() -= dust;
    });
    auto& finalreserve = _u.get(_config.getFinalReserve().value, "finalreserve not found");
...
void vigorlending::predoupdate(const uint64_t& step) {
  eosio::require_auth(get_self());
  eosio::check(_g.step == 0, "Contract is updating...");
  _g.step = step;
  _b.usern = _u.begin()->usern;
}

// distribute proxy rewards to all users having EOS proportionately
void vigorlending::dorewards() {
  ///METHOD_CONTEXT;
  eosio::asset assetin = eosio::asset(_g.rexproxy, SYSTEM_TOKEN_SYMBOL);
  auto itr = _u.lower_bound(_b.usern.value);
  auto stop_itr = _u.end();
  uint64_t counter = 0;
  if(assetin.amount < _config.getRexSwitch() * 100) {
    while(itr != stop_itr && counter++ < _b.batchsize)
      itr++;
    if(itr == _u.end()) {
      const auto rank = _u.get_index<"reputation"_n>();
  ...
  std::string message = get_self().to_string() + " has received and distributed REX income and/or voter proxy rewards: " + assetin.to_string() + " ";
  const eosio::name& userntrace = get_self();
  const uint64_t bailoutid = 0;
  eosio::asset a;
  if(_b.usern.value == _u.begin()->usern.value) {
    _b.stepdata = assetin.amount;
    _b.stepdata2 = 0;
    if(_g.getLCollaterals().hasAsset(assetin, a))
      _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(assetin, a))
      _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(assetin, a))
      _b.stepdata2 += a.amount;
    eosio::name bailoutuser = _g.bailoutuser;
    eosio::name bailoutupuser = _g.bailoutupuser;
    _b.lastflag = false;
    const auto& lastUserItr = std::find_if(_u.rbegin(), _u.rend(), [&assetin, &bailoutuser, &bailoutupuser](const auto& item) {
      return ((item.getCollaterals().hasAsset(assetin.symbol) || item.getInsurances().hasAsset(assetin.symbol)) && !(item.usern == bailoutuser || item.usern == bailoutupuser));
    });
    if(lastUserItr == _u.rend())
      _b.lastinsurer = eosio::name(0);
    else
      _b.lastinsurer = lastUserItr->usern;
  }
  if(_b.lastinsurer == eosio::name(0)) {  // no user has EOS, give rewards to finalreserve
    auto lastUseritr = _u.find(_config.getFinalReserve().value);
    const auto& lastUser = *lastUseritr;
    while(itr != stop_itr && counter++ < _b.batchsize)
      itr++;
    if(itr == _u.end()) {
      _u.modify(lastUser, get_self(), [&](auto& modified_user) {
        getInsurances(modified_user) += assetin;
        modified_user.rewardrexvot += (assetin.amount) / std::pow(10.0, assetin.symbol.precision());
      });
      _g.rexproxy = 0;
      tracetable(__func__, __LINE__, userntrace, bailoutid, message);
      const auto rank = _u.get_index<"reputation"_n>();
    ...
    uint64_t userEOSCol;
    uint64_t userEOSIns;
    auto lastUseritr = _u.find(_b.lastinsurer.value);
    const auto& lastUser = *lastUseritr;
    eosio::asset amt = eosio::asset(0, assetin.symbol);
    while(itr != stop_itr && counter++ < _b.batchsize) {
      auto& user = *itr;

      if(_b.lastflag || user.usern == _g.bailoutuser || user.usern == _g.bailoutupuser) {
        itr++;
        continue;
      }
      userEOSCol = 0;
      userEOSIns = 0;
      if(user.getCollaterals().hasAsset(assetin, a))
        userEOSCol += a.amount;
      if(user.getInsurances().hasAsset(assetin, a))
        userEOSIns += a.amount;

      if(userEOSCol > 0 && _b.stepdata > 0) {
        amt.amount = assetin.amount * (double)userEOSCol / _b.stepdata2;
        amt.amount = std::max(amt.amount, (int64_t)1);
        _u.modify(user, get_self(), [&](auto& modified_user) {
          if(user == lastUser && userEOSIns == 0)
            amt.amount = _b.stepdata;
          getCollaterals(modified_user) += amt;
          modified_user.rewardrexvot += (amt.amount) / std::pow(10.0, amt.symbol.precision());
        });
        _b.stepdata -= amt.amount;
      }

      if(userEOSIns > 0 && _b.stepdata > 0) {
        amt.amount = assetin.amount * (double)userEOSIns / _b.stepdata2;
        amt.amount = std::max(amt.amount, (int64_t)1);
        _u.modify(user, get_self(), [&](auto& modified_user) {
          if(user == lastUser)
            amt.amount = _b.stepdata;
...
void vigorlending::returnins(const eosio::name& usern) {
  ...
  for(const auto& userInsurance : asset_container_t(*userInsurances)) {
    theAssetOut = userInsurance.symbol == VIG10_SYMBOL ? eosio::asset(userInsurance.amount / std::pow(10.0, 6), VIG4_SYMBOL) : userInsurance;
    eosio::action(eosio::permission_level {get_self(), eosio::name("active")},
                  get_self(),
                  eosio::name("doassetout"),
...
void vigorlending::returncol(const eosio::name& usern) {
  ...
  for(const auto& userCollateral : asset_container_t(*userCollaterals)) {
    theAssetOut = userCollateral.symbol == VIG10_SYMBOL ? eosio::asset(userCollateral.amount / std::pow(10.0, 6), VIG4_SYMBOL) : userCollateral;
    eosio::action(eosio::permission_level {get_self(), eosio::name("active")},
                  get_self(),
                  eosio::name("doassetout"),
...
void vigorlending::exassetout(const eosio::name& usern, const eosio::asset& assetout, const std::string& memo, const bool& exec, const bool partialupdate, const uint64_t& step, double& totalval, double& totalvalbefore) {
  ...
  if(assetout.symbol == VIG4_SYMBOL)
    eosio::check(assetout.amount < INT64_MAX / std::pow(10.0, 6), "amount too large");
  const eosio::asset& localAssetOut = assetout.symbol == VIG4_SYMBOL ? eosio::asset(assetout.amount * std::pow(10.0, 6), VIG10_SYMBOL) : assetout;

  if(!exec)
    totalvalbefore = _g.valueofcol + _g.valueofins + _g.savings * ((double)_market.get(VIGOR_SYMBOL.raw()).marketdata.price[0] / pricePrecision) + (_g.l_totaldebt.amount / std::pow(10.0, _g.l_totaldebt.symbol.precision())) -
  ...
  if(!exec) {
    updateglobal();
    totalval = _g.valueofcol + _g.valueofins + _g.savings * ((double)_market.get(VIGOR_SYMBOL.raw()).marketdata.price[0] / pricePrecision) + (_g.l_totaldebt.amount / std::pow(10.0, _g.l_totaldebt.symbol.precision())) - _g.l_valueofcol - (_g.totaldebt.amount / std::pow(10.0, _g.totaldebt.symbol.precision()));
  }

  const eosio::name& userntrace = usern;
  const uint64_t bailoutid = 0;
  eosio::asset vigneededa;
  double mincollat = assetout.symbol == USDT_SYMBOL ? 1.06 : _config.getMinCollat();
  if(memo == "borrow" && assetout.symbol == VIGOR_SYMBOL) {
    ...
    eosio::check(user.valueofcol >= std::max(1.0 + (user.volcol - 0.6), 1.0) * mincollat * 0.998 * (debt.amount / std::pow(10.0, debt.symbol.precision()) + lifelinebuffer),
                 "Max allowable borrow " + eosio::asset(std::pow(10.0, debt.symbol.precision()) * std::max((user.valueofcol / (std::max(1.0 + (user.volcol - 0.6), 1.0) * mincollat)) - ((debt.amount - localAssetOut.amount) / std::pow(10.0, debt.symbol.precision())) - lifelinebuffer, 0.0), VIGOR_SYMBOL).to_string() +
                   ". Collateral Ratio limit " + std::to_string(std::max(1.0 + (user.volcol - 0.6), 1.0) * mincollat));
    ...
    // VIG lifeline for users with zero vigfees
    // Allow borrowing while at the same time the user vigfees being zero. It will invoke viglifeline; so user will borrow VIG.
    // this checks if VIGlifeline is available, but does not execute it
    auto market = _market.get(_g.fee.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : _g.fee.symbol.raw());
    double vigprice = market.marketdata.price[0] / pricePrecision;
    double vigvol = market.marketdata.vol / volPrecision;
    // estimate token swap payment due for_config.getvigLifeLine() days, at the maximum rate _config.getMaxTesPrice, since we don't know the rate yet for new borrow
    double tespay = ((user.debt.amount + localAssetOut.amount) / std::pow(10.0, user.debt.symbol.precision())) * (std::pow(1.0 + _config.getMaxTesPrice(), _config.getvigLifeLine() / 360.0) - 1.0);
    double l_tespay = user.l_valueofcol * (std::pow(1.0 + _config.getMaxTesPrice(), _config.getvigLifeLine() / 360.0) - 1.0);
    eosio::asset tespayvigor = eosio::asset(std::max((int64_t)(tespay * std::pow(10.0, VIGOR_SYMBOL.precision())), (int64_t)1), VIGOR_SYMBOL);
    eosio::asset l_tespayvigor = eosio::asset(std::max((int64_t)(l_tespay * std::pow(10.0, VIGOR_SYMBOL.precision())), (int64_t)1), VIGOR_SYMBOL);
    eosio::asset amta = eosio::asset(uint64_t(((tespayvigor.amount / std::pow(10.0, VIGOR_SYMBOL.precision())) / vigprice) * std::pow(10.0, VIG10_SYMBOL.precision())), VIG10_SYMBOL);
    eosio::asset l_amta = eosio::asset(uint64_t(((l_tespayvigor.amount / std::pow(10.0, VIGOR_SYMBOL.precision())) / vigprice) * std::pow(10.0, VIG10_SYMBOL.precision())), VIG10_SYMBOL);
    vigneededa = amta + l_amta;
    if(!exec && user.vigfees <= 0) {
      double vigneeded = vigneededa.amount / std::pow(10.0, vigneededa.symbol.precision()) * vigprice;
      double overcol = 1.0 + 2.0 * vigvol / sqrt(12);  // overcollateralize the VIG borrow by this amount, enough collateral to survive a 2 * monthly standard deviation move of VIG price
      auto& finalreserve = _u.get(_config.getFinalReserve().value, "finalreserve not found");
      eosio::asset lr;
      eosio::asset vigfees = eosio::asset(user.vigfees, VIG10_SYMBOL);
      finalreserve.getInsurances().hasAsset(vigfees, lr);
      eosio::asset ginsurance;
      // check if VIG lifeline is available, and user has capacity to borrow VIG
      eosio::asset vigorneededa = eosio::asset((int64_t)(overcol * vigneeded * std::pow(10.0, VIGOR_SYMBOL.precision())), VIGOR_SYMBOL);
      eosio::asset c;
      eosio::asset d;
      eosio::asset globalInsurance;
      bool lifeline = false;  // VIG lifeline not available
      if((user.getCollaterals().hasAsset(_g.fee, c) && c.amount >= vigneededa.amount && (user.valueofcol - vigneeded >= 1.02 * ((user.debt.amount + localAssetOut.amount) / std::pow(10.0, user.debt.symbol.precision())))) ||
         (user.getInsurances().hasAsset(_g.fee, d) && d.amount >= vigneededa.amount && _g.getInsurances().hasAsset(_g.fee, globalInsurance) && globalInsurance.amount >= vigneededa.amount))
        lifeline = true;
      if(((_g.getInsurances().hasAsset(vigfees, ginsurance) && (ginsurance.amount - lr.amount >= vigneededa.amount)) &&
          (((((user.debt.amount + localAssetOut.amount) / std::pow(10.0, user.debt.symbol.precision())) + overcol * vigneeded) <= user.valueofcol) ||
           ((user.l_valueofcol + vigneeded) <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision()))))))
        lifeline = true;
      eosio::check(lifeline, "Zero balance not allowed for VIG Fees. Please deposit VIG, it is needed to pay the fees over time.");
    }
    ...
    if(memo == "insurance") {
      ...
      double valueofasset = localAssetOut.amount / std::pow(10.0, insurance.symbol.precision());
      auto market = _market.get(localAssetOut.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : localAssetOut.symbol.raw());

      valueofasset *= (double)market.marketdata.price[0] / pricePrecision;
      ...
      if(!globalInsurances.hasAsset(localAssetOut, globalinsurance)) {
        lendavail = localAssetOut.symbol == VIG10_SYMBOL ? eosio::asset(0, VIG4_SYMBOL) : eosio::asset(0, localAssetOut.symbol);
      ...
      lendavail = globalinsurance.symbol == VIG10_SYMBOL ? eosio::asset(globalinsurance.amount / std::pow(10.0, 6), VIG4_SYMBOL) : globalinsurance;
      ...
      if(exec) {
        ...
        if(assetout.symbol == VIG4_SYMBOL && (insurance.amount - localAssetOut.amount) < 1000000 && (insurance.amount - localAssetOut.amount) > 0) {
          eosio::asset dust = insurance - localAssetOut;
          _u.modify(user, get_self(), [&](auto& modified_user) {
            modified_user.getInsurances() -= dust;
          });
          auto& finalreserve = _u.get(_config.getFinalReserve().value, "finalreserve not found");
    ...
    if(memo == "savings") {
      ...
      double valueofasset = localAssetOutDouble * (double)market.marketdata.price[0] / pricePrecision;
      ...
      double valueofasset = localAssetOut.amount / std::pow(10.0, collateral.symbol.precision());
      auto market = _market.get(localAssetOut.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : localAssetOut.symbol.raw());
      ...
      asset_container usercol = user.getCollaterals();
      usercol -= localAssetOut;
      double volcol = sqrt(portfolioVariance(*usercol, std::max(valueofcol, 0.0)));
      usercol += localAssetOut;
      if(!(valueofcol >= std::max(1.0 + (volcol - 0.6), 1.0) * mincollat * 0.998 * (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())))) {
        eosio::asset x = bisection(eosio::asset(0, collateral.symbol), eosio::asset(std::pow(10.0, collateral.symbol.precision()) * (std::min((collateral.amount / std::pow(10.0, collateral.symbol.precision())) * ((double)market.marketdata.price[0] / pricePrecision), std::max((user.valueofcol - mincollat * (user.debt.amount / std::pow(10.0, user.debt.symbol.precision()))), 0.0)) / ((double)market.marketdata.price[0] / pricePrecision)), collateral.symbol), user, true);
        usercol -= x;
        volcol = sqrt(portfolioVariance(*usercol, std::max(user.valueofcol - (x.amount / std::pow(10.0, x.symbol.precision())) * ((double)market.marketdata.price[0] / pricePrecision), 0.0)));
        usercol += x;
        x = x.symbol == VIG10_SYMBOL ? eosio::asset(x.amount / std::pow(10.0, 6), VIG4_SYMBOL) : x;
        eosio::check(false, "Max allowable collateral withdraw " + x.to_string() +
                              ". Collateral Ratio limit " + std::to_string(std::max(1.0 + (volcol - 0.6), 1.0) * mincollat));
      }
      ...
      if(exec) {
        _g.volume += valueofasset;
        if(assetout.symbol == VIG4_SYMBOL && (collateral.amount - localAssetOut.amount) < 1000000 && (collateral.amount - localAssetOut.amount) > 0) {
          eosio::asset dust = collateral - localAssetOut;
          _u.modify(user, get_self(), [&](auto& modified_user) {
            modified_user.getCollaterals() -= dust;
          });
          auto& finalreserve = _u.get(_config.getFinalReserve().value, "finalreserve not found");
      ...
      double valueofasset = localAssetOut.amount / std::pow(10.0, vigfees.symbol.precision());
      auto market = _market.get(localAssetOut.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : localAssetOut.symbol.raw());
      ...
      if(user.isBorrower())
        eosio::check((uint64_t)(vigfees.amount / std::pow(10.0, 6)) - (uint64_t)(localAssetOut.amount / std::pow(10.0, 6)) >= (uint64_t)(_config.getInitialVig() * std::pow(10.0, VIG4_SYMBOL.precision())), "User has debt. Not allowed to withdraw vigfees below " + eosio::asset((uint64_t)(_config.getInitialVig() * std::pow(10.0, VIG4_SYMBOL.precision())), VIG4_SYMBOL).to_string() + " during borrow. Available withdraw: " + eosio::asset(std::max((double)((int64_t)(vigfees.amount / std::pow(10.0, 6)) - (int64_t)(_config.getInitialVig() * std::pow(10.0, VIG4_SYMBOL.precision()))), 0.0), VIG4_SYMBOL).to_string());
      ...
      auto market = _market.get(localAssetOut.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : localAssetOut.symbol.raw());
      double valueofassetout = (localAssetOut.amount) / std::pow(10.0, localAssetOut.symbol.precision()) *
                               ((double)market.marketdata.price[0] / pricePrecision);

      //eosio::check( user.txnvolume + valueofassetout <= _config.getDataA(), "Total daily outbound transfers cannot exceed " + std::to_string(_config.getDataA()) + " dollars worth of tokens." );
      double lifelinebuffer = 0.0;
      if(!exec && user.vigfees <= 0)
        lifelinebuffer = 0.0003;

      asset_container userlcol = user.getLCollaterals();
      userlcol += localAssetOut;
      double l_volcol = sqrt(portfolioVariance(*userlcol, user.l_valueofcol + valueofassetout));
      userlcol -= localAssetOut;
      if(!(user.l_valueofcol + valueofassetout + lifelinebuffer <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())) / (std::max(1.0 + (l_volcol - 0.6), 1.0) * mincollat * 0.998))) {
        eosio::asset x = bisection(eosio::asset(0, localAssetOut.symbol), eosio::asset(std::pow(10.0, localAssetOut.symbol.precision()) * (std::max(((user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())) / mincollat - user.l_valueofcol), 0.0) / ((double)market.marketdata.price[0] / pricePrecision)), localAssetOut.symbol), user, false);
        userlcol += x;
        l_volcol = sqrt(portfolioVariance(*userlcol, user.l_valueofcol + (x.amount / std::pow(10.0, x.symbol.precision())) * ((double)market.marketdata.price[0] / pricePrecision)));
        userlcol -= x;
        x = x.symbol == VIG10_SYMBOL ? eosio::asset(x.amount / std::pow(10.0, 6), VIG4_SYMBOL) : x;
        x.amount = (int64_t)(std::pow(10.0, x.symbol.precision()) * (std::max((x.amount / std::pow(10.0, x.symbol.precision())) * ((double)market.marketdata.price[0] / pricePrecision) - lifelinebuffer, 0.0)) / ((double)market.marketdata.price[0] / pricePrecision));
        eosio::check(false, "Max allowable crypto borrow " + x.to_string() +
                              ". Collateral Ratio limit " + std::to_string(std::max(1.0 + (l_volcol - 0.6), 1.0) * mincollat));
      }
      ...
      auto symbolIterator = _whitelist.find(localAssetOut.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : localAssetOut.symbol.raw());
      ...
      eosio::asset remborrow;
      if(localAssetOut.symbol == VIG10_SYMBOL)
        remborrow = eosio::asset(std::max(localAssetOut.amount + maxBorrowAmount, (int64_t)0) / std::pow(10.0, 6), VIG4_SYMBOL);
      else
        remborrow = eosio::asset(std::max(localAssetOut.amount + maxBorrowAmount, (int64_t)0), localAssetOut.symbol);
      eosio::check(maxBorrowAmount >= 0, std::string("Can't locate enough to borrow. Remaining available to borrow: " + remborrow.to_string()));

      // VIG lifeline for users with zero vigfees
      // Allow borrowing while at the same time the user vigfees being zero. It will invoke viglifeline; so user will borrow VIG.
      // this checks if VIGlifeline is available, but does not execute it
      market = _market.get(VIG4_SYMBOL.raw());
      double vigprice = market.marketdata.price[0] / pricePrecision;
      double vigvol = market.marketdata.vol / volPrecision;
      // estimate token swap payment due for_config.getvigLifeLine() days, at the maximum rate _config.getMaxTesPrice, since we don't know the rate yet for new borrow
      double tespay = (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) * (std::pow(1.0 + _config.getMaxTesPrice(), _config.getvigLifeLine() / 360.0) - 1.0);
      double l_tespay = (user.l_valueofcol + valueofassetout) * (std::pow(1.0 + _config.getMaxTesPrice(), _config.getvigLifeLine() / 360.0) - 1.0);
      eosio::asset tespayvigor = eosio::asset(std::max((int64_t)(tespay * std::pow(10.0, VIGOR_SYMBOL.precision())), (int64_t)1), VIGOR_SYMBOL);
      eosio::asset l_tespayvigor = eosio::asset(std::max((int64_t)(l_tespay * std::pow(10.0, VIGOR_SYMBOL.precision())), (int64_t)1), VIGOR_SYMBOL);
      eosio::asset amta = eosio::asset(uint64_t(((tespayvigor.amount / std::pow(10.0, VIGOR_SYMBOL.precision())) / vigprice) * std::pow(10.0, VIG10_SYMBOL.precision())), VIG10_SYMBOL);
      eosio::asset l_amta = eosio::asset(uint64_t(((l_tespayvigor.amount / std::pow(10.0, VIGOR_SYMBOL.precision())) / vigprice) * std::pow(10.0, VIG10_SYMBOL.precision())), VIG10_SYMBOL);
      ...
      if(!exec && user.vigfees <= 0) {
        ...
        eosio::asset globalInsurance;
        bool lifeline = false;  // VIG lifeline not available
        if((user.getCollaterals().hasAsset(_g.fee, c) && c.amount >= vigneededa.amount && (user.valueofcol - vigneeded >= 1.02 * (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())))) ||
           (user.getInsurances().hasAsset(_g.fee, d) && d.amount >= vigneededa.amount && _g.getInsurances().hasAsset(_g.fee, globalInsurance) && globalInsurance.amount >= vigneededa.amount))
          lifeline = true;
        if(((_g.getInsurances().hasAsset(vigfees, ginsurance) && (ginsurance.amount - lr.amount >= vigneededa.amount)) &&
            ((((user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) + overcol * vigneeded) <= user.valueofcol) ||
             ((user.l_valueofcol + vigneeded + valueofassetout) <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision()))))))
          lifeline = true;
        eosio::check(lifeline, "Zero balance not allowed for VIG Fees. Please deposit VIG, it is needed to pay the fees over time.");
      }

      if(!exec)
        totalval -= valueofassetout;

      if(exec) {
        _g.volume += valueofassetout;
        _u.modify(user, get_self(), [&](auto& modified_user) {
          getLCollaterals(modified_user) += localAssetOut;
          //modified_user.txnvolume += valueofassetout;
        });
      }
      ...
      // if overcollateralization is C then leverage L = 1 / ( 1 - ( 1 / C ) )
      eosio::check(user.l_valueofcol * std::max(1.0 + (user.l_volcol - 0.6), 1.0) * mincollat * 0.998 <= (l_debt.amount / std::pow(10.0, l_debt.symbol.precision())),
                   "Max allowable collateral withdraw " + eosio::asset(std::pow(10.0, l_debt.symbol.precision()) * std::max(((user.l_debt.amount / std::pow(10.0, l_debt.symbol.precision())) - (user.l_valueofcol * std::max(1.0 + (user.l_volcol - 0.6), 1.0) * mincollat)), 0.0), VIGOR_SYMBOL).to_string() +
                     ". Collateral Ratio limit " + std::to_string(std::max(1.0 + (user.l_volcol - 0.6), 1.0) * mincollat));

      double valueofasset = (localAssetOut.amount / std::pow(10.0, localAssetOut.symbol.precision()));
      ...
      if(exec) {
        ...
        if((double)lent / S0 < 0.898 && rex.amount > 0) {
          ...
          eosio::action(eosio::permission_level {get_self(), "active"_n},
                        eosio::name("eosio"),
                        eosio::name("withdraw"),
                        std::make_tuple(get_self(), localAssetOut))
            .send();
    ...
    if(exec) {
      const eosio::asset& lastAssetOut = localAssetOut.symbol == VIG10_SYMBOL ? eosio::asset(localAssetOut.amount / std::pow(10.0, 6), VIG4_SYMBOL) : localAssetOut;
    ...
    switch(_config.getExecType()) {
      ...
      case 2:  //mining
        ...
        if(partialupdate) {  // partial update
          ...
          _g.l_premiums -= l_premiums;
          l_pricing(user);
          if(user.vigfees <= 0 && memo == "borrow") {
            eosio::asset vigfees = eosio::asset(user.vigfees, VIG10_SYMBOL);
            viglifeline(user, vigfees, vigneededa, vigprice, vigvol);
            eosio::check(vigfees.amount > 0, "Deposit " + eosio::asset((uint64_t)(_config.getInitialVig() * std::pow(10.0, VIG4_SYMBOL.precision())) - (uint64_t)(vigfees.amount / std::pow(10.0, 6)), VIG4_SYMBOL).to_string() + " into VIG Fees (or borrow some VIG) it is needed to pay the borrow fees over time.");
            update(user);
            updateglobal();
          }
          _g.rm -= dRM;
...
double vigorlending::portfolioVariance(asset_container_t& container, double valueOf) {
  ...
  for(auto asset = container.begin(); asset != container.end(); ++asset) {
    if(i == 0) {
      m[i] = &(_market.get(asset->symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : asset->symbol.raw()).marketdata);
      w[i] = ((*m[i]).price[0] / pricePrecision) * (asset->amount / std::pow(10.0, asset->symbol.precision())) / valueOf;
    }
    int j = i;
    double p = 0.0;
    for(auto nextAsset = asset + 1; nextAsset != container.end(); ++nextAsset) {
      j++;
      if(i == 0) {
        m[j] = &(_market.get(nextAsset->symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : nextAsset->symbol.raw()).marketdata);
        w[j] = ((*m[j]).price[0] / pricePrecision) * (nextAsset->amount / std::pow(10.0, nextAsset->symbol.precision())) / valueOf;
      }
      double c = (*m[i]).correlation_matrix.at(nextAsset->symbol == VIG10_SYMBOL ? VIG4_SYMBOL : nextAsset->symbol) / corrPrecision;  // corr matrix has VIG 4 not VIG 10
      p += w[j] * c * (*m[j]).vol;
    }
  ...
  for(auto asset = container.begin(); asset != container.end(); ++asset) {
    if(i == 0) {
      ...
      if(userInsurances.hasAsset(*asset, userInsurance)) {
        user_valueofins_adj = ((*m[i]).price[0] / pricePrecision) * (userInsurance.amount / std::pow(10.0, asset->symbol.precision()));
      ...
      w[i] = std::max((((*m[i]).price[0] / pricePrecision) * (asset->amount / std::pow(10.0, asset->symbol.precision())) - user_valueofins_adj), 0.0) / valueOf;
    }
    int j = i;
    double p = 0.0;
    for(auto nextAsset = asset + 1; nextAsset != container.end(); ++nextAsset) {
      ...
      if(i == 0) {
        ...
        if(userInsurances.hasAsset(*nextAsset, userInsurance)) {
          ua = ((*m[j]).price[0] / pricePrecision) * (userInsurance.amount / std::pow(10.0, nextAsset->symbol.precision()));
        ...
        w[j] = std::max((((*m[j]).price[0] / pricePrecision) * (nextAsset->amount / std::pow(10.0, nextAsset->symbol.precision())) - ua), 0.0) / valueOf;
...
void vigorlending::pricing(const user_s& user) {
  ...
  double payoff = std::max(0.0,
                           1.0 * (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) - user.valueofcol * (1.0 - istresscol));
  double T = 1.0;
  double d = ((std::log(user.valueofcol / (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())))) + (-std::pow(ivol, 2) / 2.0) * T * _config.getDataC()) / (ivol * std::sqrt(T * _config.getDataC()));

  double discount = 1.0 - user.getReputation_pct() * _config.getMaxDisc();

  double tesprice = 0.0;
  if(user.debt.amount > 0)
    tesprice = discount * std::min(std::max(_config.getMinTesPrice(),
                                            ((payoff * std::erfc(d / std::sqrt(2.0)) / 2.0) / (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())))),
                                   _config.getMaxTesPrice());

  double premiums = tesprice * (user.debt.amount / std::pow(10.0, user.debt.symbol.precision()));

  _g.premiums += premiums;  // total dollar amount of premiums all borrowers would pay in one year to insure their collateral

  _u.modify(user, get_self(), [&](auto& modified_user) {
    if(modified_user.tesprice != tesprice)
...
void vigorlending::l_pricing(const user_s& user) {
  ...
  for(const auto& l_collateral : *user.getLCollaterals()) {
    auto market = _market.get(l_collateral.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : l_collateral.symbol.raw());
    double l_valueofcol = (l_collateral.amount) / std::pow(10.0, l_collateral.symbol.precision()) *
                          ((double)market.marketdata.price[0] / pricePrecision);
    auto l_col = _whitelist.find(l_collateral.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : l_collateral.symbol.raw());
  ...
  double payoff = std::max(0.0,
                           1.0 * user.l_valueofcol * (1.0 + l_istresscol) - (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())));
  double T = 1.0;
  double d = ((std::log(user.l_valueofcol / (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())))) + (-std::pow(l_ivol, 2) / 2.0) * T * _config.getDataC()) / (l_ivol * std::sqrt(T * _config.getDataC()));

  double discount = 1.0 - user.getReputation_pct() * _config.getMaxDisc();

  double l_tesprice = 0.0;
  if(user.l_valueofcol > 0)
    l_tesprice = discount * std::min(std::max(_config.getMinTesPrice(),
                                              ((payoff * std::erfc((-1.0 * d) / std::sqrt(2.0)) / 2.0) / user.l_valueofcol)),
                                     _config.getMaxTesPrice());

  double l_premiums = l_tesprice * user.l_valueofcol;

  _g.l_premiums += l_premiums;  // total dollar amount of premiums all borrowers would pay in one year to insure their collateral

  _u.modify(user, get_self(), [&](auto& modified_user) {
    if(modified_user.l_tesprice != l_tesprice)
...
void vigorlending::l_pcts(const user_s& user) {
  ...
  if(user.valueofins <= 0.0 || _b.l_rmliq <= 0.0) {  // insurer bears no bailout risk nor shall be compensated, so l_pctl is set to zero.
    ...
    for(const auto& userIns : *user.getInsurances()) {
      auto ins = _whitelist.find(userIns.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : userIns.symbol.raw());
...
void vigorlending::distribfee() {
  ...
  //  Find the last eligible user to receive rewards, so we can give dust
  eosio::name bailoutuser = _g.bailoutuser;
  eosio::name bailoutupuser = _g.bailoutupuser;
  ...
  if(_b.usern.value == _u.begin()->usern.value) {
    _b.lastflag = false;
    const auto& lastInsurer = *std::find_if(_u.rbegin(), _u.rend(), [&bailoutuser, &bailoutupuser, &finalreserve, &vigordacfund](const auto& user) {
      return user.isEligible(bailoutuser, bailoutupuser, finalreserve, vigordacfund);
    });
    _b.lastinsurer = lastInsurer.usern;
    _b.stepdata = (_g.fee.amount + _g.l_fee.amount);
  }
  ...
  while(itr != stop_itr && counter++ < _b.batchsize) {
    if(_b.lastflag || (_g.fee.amount + _g.l_fee.amount <= 0)) {
      itr++;
      continue;
    }
    ...
    vigsavings.amount = _g.fee.amount * savings_pct + _g.l_fee.amount * savings_pct;

    if(user == lastInsurer)
      viga.amount = _b.stepdata;  // remaining amount, dust
    viga.amount = std::max(viga.amount - vigsavings.amount, (int64_t)(0));

    // if insurer has some amount of VIG in their insurance, then reward them for that amount, but must be denominated in VIGOR and originate from vigordacfund as an expense
    // to accomplish that, send the VIG premiums into vigordacfund as collateral, then borrow VIGOR, and pay the user in VIGOR into savings
    // if insurer has other tokens in their insurance such as EOS, then pay the user for that amount, denominated in VIG as usual into vigfees
    ...
    if(user.getInsurances().hasAsset(_g.fee, ins) && viga.amount > 0 && user.usern.value != _config.getFinalReserve().value) {
      double insval = ins.amount / std::pow(10.0, ins.symbol.precision()) * vigprice;
      double pct = std::min(insval / user.valueofins, 1.0);                                   // amount of VIG in the user insurance as a percentage of the total dollar value of their insurance
      double oc = 1.0;                                                                        // +2.0 * vigvol / sqrt(12); // overcollateralize the VIGOR borrow by this amount, enough collateral to survive a 2 * monthly standard deviation move of VIG price
      ...
      if(pctamta.amount > 0) {                                                                // amount may be too small, may become zero when cast
        ...
        pctamtatotal += pctamta;                                                              // note if vigorpay.amount rounds to zero, then the user gives the dust amount of VIG pctamta to vigordacfund for nothing in return. user is no allowed to keep VIG on VIG, not even dust amounts
    ...
    _u.modify(user, get_self(), [&](auto& modified_user) {
      if(user.usern == _config.getFinalReserve()) {
        getInsurances(modified_user) += viga + vigsavings;  // reserve receives the fee into insurance and also adds to global insurance, the reserve does not pay any fees so it needs no VIG in vigfees.
      } else {
        modified_user.vigfees += viga.amount + vigsavings.amount;  // has an adjustment for dust, so that the amount allocated to last insurer brings the vigaremaining to zero. VIG rewards are paid into user vigfees, if they recieve bailouts they have VIG in vigfees to pay the fees.
        _g.vigfees += viga.amount + vigsavings.amount;
        if(vigorpay.amount > 0) {
          ...
          modified_user.rewardlend2.utc_seconds += vigorpay.amount;
        }
      }
      double days_since_anniv = (3600.0 + eosio::current_time_point().sec_since_epoch() - (modified_user.getAnniv().sec_since_epoch() - _config.getRepAnniv())) / eosio::days(1).to_seconds();
      uint64_t rep = std::pow(10.0, _g.fee.symbol.precision()) * std::sqrt(std::pow(modified_user.getVig_since_anniv().amount / std::pow(10.0, _g.fee.symbol.precision()), 2.0) + (i_reputation(viga + vigsavings + pctamta).amount / std::pow(10.0, _g.fee.symbol.precision())));
      modified_user.setRep_lag0(eosio::asset(rep / std::sqrt(days_since_anniv), _g.fee.symbol));
      modified_user.setVig_since_anniv(eosio::asset(rep, _g.fee.symbol));
      modified_user.rewardlend += viga.amount / std::pow(10.0, viga.symbol.precision());
      modified_user.rewardsave += vigsavings.amount / std::pow(10.0, vigsavings.symbol.precision());
      if(days_since_anniv > (double)_config.getRepAnniv() / eosio::days(1).to_seconds()) {
        modified_user.reputationrotation();
        modified_user.setVig_since_anniv(eosio::asset(0, _g.fee.symbol));
        modified_user.setAnniv(eosio::current_time_point(), _config.getRepAnniv());
    ...
    _b.stepdata -= (viga.amount + vigsavings.amount + pctamta.amount);
    itr++;
  }
  ...
  _u.modify(dac, get_self(), [&](auto& dac) {
    getCollaterals(dac) += pctamtatotal;  // note if vigorpay.amount rounds to zero, then the user gives the dust amount of VIG pctamta to vigordacfund for nothing in return. user is no allowed to keep VIG on VIG, not even dust amounts
...
void vigorlending::update(const user_s& user) {
  ...
  for(const auto& insurance : *user.getInsurances()) {
    auto market = _market.get(insurance.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : insurance.symbol.raw());
    valueofins += (insurance.amount) / std::pow(10.0, insurance.symbol.precision()) *
                  ((double)market.marketdata.price[0] / pricePrecision);
  }

  for(const auto& collateral : *user.getCollaterals()) {
    auto market = _market.get(collateral.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : collateral.symbol.raw());
...
void vigorlending::updateglobal() {
  ...
  auto market = _market.get(VIGOR_SYMBOL.raw());
  if(_g.savingsscale != std::max(std::min(std::pow(1.0 / ((double)market.marketdata.price[0] / pricePrecision), 4.0), 3.0), 0.33))
    _g.savingsscale = std::max(std::min(std::pow(1.0 / ((double)market.marketdata.price[0] / pricePrecision), 4.0), 3.0), 0.33);
  ...
  while(itr != _whitelist.end()) {
    eosio::asset a = eosio::asset(0, itr->sym == VIG4_SYMBOL ? VIG10_SYMBOL : itr->sym);
    eosio::asset i = a;
    eosio::asset r = a;
    eosio::asset l_col = a;
...
void vigorlending::performance(const user_s& user, const double& vigvol, const double& vigprice) {
  ...
  eosio::asset ins = eosio::asset(0, _g.fee.symbol);
  double haircut = 1.0;  // VIG on VIG taboo haircut
  if(user.getInsurances().hasAsset(_g.fee, ins) && user.usern.value != _config.getFinalReserve().value && user.usern.value != _config.getVigorDacFund().value) {
    double insval = ins.amount / std::pow(10.0, ins.symbol.precision()) * vigprice;
    double pct = insval / user.valueofins;  // amount of VIG in the user insurance as a percentage of the total dollar value of their insurance
    double oc = 1.0;                        // + 2.0 * vigvol / sqrt(12); // overcollateralize the VIGOR borrow by this amount, enough collateral to survive a 2 * monthly standard deviation move of VIG price
    haircut = 1.0 - pct * (1.0 - 1.0 / oc);
  }
...
void vigorlending::performanceglobal() {
  ...
  if(_g.valueofins > 0.0)
    _g.earnrate = std::min(((_g.premiums + _g.l_premiums) * (1.0 - _config.getReserveCut() - _config.getVigorDacCut() - _config.getSavingsCut() * _g.savingsscale)) / (_g.valueofins + _g.l_valueofcol), 1.0);  // annualized rate of return on total portfolio of insurance crypto assets, insuring for downside and upside price jumps

  _g.savingsrate = 0.0;

  if(_g.savings > 0.0)
    _g.savingsrate = std::min((_config.getSavingsCut() * _g.savingsscale * (_g.premiums + _g.l_premiums)) / _g.savings, 1.0);  // annualized rate of return on VIGOR deposited into l_debt, savings account, risk free earings
}
...
void vigorlending::bailout(const eosio::name& usern) {
  ...
  if(_b.usern.value == _u.begin()->usern.value) {
    ...
    // if reserve is being bailed out by savers, the savers get the collateral at a 10% discount
    // if(usern.value == res.usern.value) _b.excesspct *= 1.1;
    ...
    if(_b.excesspct < 1.0) {
      ...
      message.append(std::string("Borrower sent to bailout : " + user.usern.to_string() + " liquidated debt: " + user.debt.to_string() + " liquidated collateral: (dollar value) " + std::to_string(user.valueofcol) + " " + user.getCollaterals().to_string() + ". user retains excess collateral: (dollar value) " + std::to_string(user.valueofcol / _b.excesspct - user.valueofcol) + " "));
      for(auto col : _b.assetcont)
        message.append(std::string(col.to_string() + " "));
    } else {
      message.append(std::string("Borrower sent to bailout : " + user.usern.to_string() + " liquidated debt: " + user.debt.to_string() + " liquidated collateral: (dollar value) " + std::to_string(user.valueofcol) + " " + user.getCollaterals().to_string()));
    }
    ...
    if(_b.stepdata2 == 1) {
      ...
      if(user.usern.value != _config.getFinalReserve().value && user.pcts / _g.pcts == 1.0) {
        ...
        const auto& lastInsurer = *std::find_if(_u.rbegin(), _u.rend(), [&](const auto& item) { return item.pcts > 0 && item != user; });
        _b.lastinsurer = lastInsurer.usern;
      }
    }
  }
  ...
  if(itr == _u.end()) {
    ...
    if(_b.excesspct < 1.0) {
      ...
      for(const auto& userExcessCollateral : _b.assetcont) {
        _u.modify(user, get_self(), [&](auto& modified_user) {
          userCollaterals += userExcessCollateral;
        });
      }
      _u.modify(user, get_self(), [&](auto& modified_user) {
...
void vigorlending::recap(const user_s& user, const user_s& insurerUser, double insurerpcts, double& sumpcts, bool lastInsurer) {
  ...
  // recap method 1: if insurer has VIGOR then use it to cancel insurers share of the bad debt
  double valueofins = insurerUser.valueofins + user.valueofcol * insurerpcts;  //adjusted value of insurance to include the newly acquired collateral
  eosio::symbol vigorSymbol = VIGOR_SYMBOL;                                    //  TODO: This should be global to use on the whole code when needed
  ...
  if(!(_b.stepdata2 == 1)) {
    if(insurerInsurances.hasAsset(vigorSymbol)) {
      _u.modify(_u.iterator_to(insurerUser), get_self(), [&](auto& modified_user) {
        ...
        insurerInsurances -= amt;
        modified_user.debt -= amt;

        _g.getInsurances() -= amt;
  ...
  double recapReq = std::min((debtshare.amount / std::pow(10.0, debtshare.symbol.precision())) * (1.0 + 2.0 * sp) / valueofins, 1.0);  // recapReq as a percentage of the insurers insurance assets, size to cover a 2 sigma monthly move
  ...
  for(const auto& insurance : asset_container_t(*insurerInsurances)) {
    ...
    amt.amount *= recapReq;
    amt.amount = std::max(amt.amount, (int64_t)1);
    eosio::asset blocked = amt;  // if token amount breaches maxlends limit, then insurers can't withdraw nor convert to collateral for recap

    auto globalInsurances = _g.getInsurances();
    eosio::asset globalInsurance;
    if(globalInsurances.hasAsset(insurance, globalInsurance)) {
...
void vigorlending::recapup(const user_s& user, const user_s& insurerUser, double insurerlpcts, double& sumpcts, bool lastInsurer) {
  ...
  double W1 = user.l_valueofcol * insurerlpcts;
  auto userLCollaterals = getLCollaterals(const_cast<user_s&>(user));

  if((*userLCollaterals).empty())
    W1 = 0;  // insurer share of the l_collateral
  ...
  for(const auto& userLCollateral : asset_container_t(*userLCollaterals)) {
    ...
    _u.modify(user, get_self(), [&](auto& modified_user) {
      if(lastInsurer)
        amt = userLCollateral;
      userLCollaterals -= amt;
    });
    // add borrows to insurers l_collateral
    _u.modify(_u.iterator_to(insurerUser), get_self(), [&](auto& modified_user) {
      getLCollaterals(modified_user) += amt;
    });
    ...
    for(const auto& userInsurance : asset_container_t(*insurerUser.getInsurances())) {
      if(userInsurance.symbol == amt.symbol) {
        eosio::asset amti = userInsurance;
        amti.amount = std::min(amti.amount, amt.amount);
        recap1_t.push_back(amti);
        auto market = _market.get(userInsurance.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : userInsurance.symbol.raw());
        valueofins -= (amti.amount) / std::pow(10.0, userInsurance.symbol.precision()) * ((double)market.marketdata.price[0] / pricePrecision);  //adjusted value of insurance to include cancelling some of the borrow
        W1 -= (amti.amount) / std::pow(10.0, userInsurance.symbol.precision()) * ((double)market.marketdata.price[0] / pricePrecision);          //adjusted value of the share of l_collateral to include cancelling some of the borrow
  ...
  double vigorneeded = std::max((W1 * (1.0 + 2.0 * sp) - (l_debtshare.amount / std::pow(10.0, l_debtshare.symbol.precision()))), 0.0);  // need this many VIGOR to recap the borrowed risky tokens, enough to cover a 2 monthly standard deviation upward move
  eosio::asset vigorneededa = eosio::asset((uint64_t)(vigorneeded * std::pow(10.0, VIGOR_SYMBOL.precision())), VIGOR_SYMBOL);
  if(vigorneededa.amount <= 0) {
    if(user.usern.value != _config.getFinalReserve().value)
      _bailout.emplace(get_self(), [&](auto& b) {
        b.id = _bailout.available_primary_key();
        b.timestamp = eosio::current_time_point();
  ...
  for(const auto& userInsurance : asset_container_t(*insurerUser.getInsurances())) {
    if(userInsurance.symbol == VIGOR_SYMBOL) {
      ...
      // subtract VIGOR from insurers insurance, global insurance, and add to insurers l_debt and global l_debt
      _u.modify(_u.iterator_to(insurerUser), get_self(), [&](auto& modified_user) {
        getInsurances(modified_user) -= amt;
  ...
  // recapReq: amount of collateral needed to recap the risky debt (enough to cover a 2 monthly std upward move), against which the insurer can borrow VIGOR to recap the borrowed risky tokens, as a pct of insurers insurance
  double recapReq = std::min(((1.0 + 2.0 * spi) * (vigorneededa.amount / std::pow(10.0, vigorneededa.symbol.precision()))) / valueofins, 1.0);  // recap needed as a pct of how much is available
  auto symbolIterator = _whitelist.find(VIGOR_SYMBOL.raw());
  currency_statst _currency_statst(symbolIterator->contract, VIGOR_SYMBOL.code().raw());
  auto existing = _currency_statst.find(VIGOR_SYMBOL.code().raw());
  if(existing != _currency_statst.end()) {
    const auto& st = *existing;
    if(st.supply.amount + vigorneededa.amount * (1.0 + 2.0 * spi) > _config.getDebtCeiling() * std::pow(10.0, VIGOR_SYMBOL.precision()))
      return;
  }
  // subtract the recap amount from the insurers insurance, and global insurance. And add it to insurers collateral and global collateral
  ...
  double recapAvail = std::min(std::max(valueofins, 0.0) / ((1.0 + 2.0 * spi) * (vigorneededa.amount / std::pow(10.0, vigorneededa.symbol.precision()))), 1.0);  // recap available as a pct of how much is needed, after blocking is considered
  vigorneededa.amount *= recapAvail;
  if(vigorneededa.amount > 0) {
    // recap available as a pct of how much is needed, recapAvail
    ...
    if(user.usern.value != _config.getFinalReserve().value)
      _bailout.emplace(get_self(), [&](auto& b) {
        ...
        b.recap2 = recap2_t;
        b.recap3 = recap3_t;
        b.recap3b.push_back(vigorneededa);
        b.blockedins = blockedins_t;
...
double vigorlending::func(eosio::asset x, const user_s& user, const market_s& market, bool txntype) {
  double valueofasset = x.amount / std::pow(10.0, x.symbol.precision());
  valueofasset *= (double)market.marketdata.price[0] / pricePrecision;
  double mincollat = x.symbol == USDT_SYMBOL ? 1.06 : _config.getMinCollat();
  if(txntype) {
    double valueofcol = user.valueofcol - valueofasset;
    asset_container usercol = user.getCollaterals();
    eosio::asset _x = x.symbol == VIG4_SYMBOL ? eosio::asset(x.amount * std::pow(10.0, 6), VIG10_SYMBOL) : x;
    usercol -= _x;
    double volcol = sqrt(portfolioVariance(*usercol, std::max(valueofcol, 0.0)));
    usercol += _x;
    return valueofcol - (std::max(1.0 + (volcol - 0.6), 1.0) * mincollat) * (user.debt.amount / std::pow(10.0, user.debt.symbol.precision()));
  } else {
    double l_valueofcol = user.l_valueofcol + valueofasset;
    asset_container userlcol = user.getLCollaterals();
    eosio::asset _x = x.symbol == VIG4_SYMBOL ? eosio::asset(x.amount * std::pow(10.0, 6), VIG10_SYMBOL) : x;
    userlcol += x;
    double l_volcol = sqrt(portfolioVariance(*userlcol, l_valueofcol));
    userlcol -= x;
    return ((user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())) / (std::max(1.0 + (l_volcol - 0.6), 1.0) * mincollat)) - l_valueofcol;
  }
}

eosio::asset vigorlending::bisection(eosio::asset a, eosio::asset b, const user_s& user, bool txntype) {
  a = a.symbol == VIG10_SYMBOL ? eosio::asset(a.amount / std::pow(10.0, 6), VIG4_SYMBOL) : a;
  b = b.symbol == VIG10_SYMBOL ? eosio::asset(b.amount / std::pow(10.0, 6), VIG4_SYMBOL) : b;
  eosio::asset c = a;
  double cval, aval = 0.0;
  int count = 0;
  auto market = _market.get(a.symbol.raw());
  double bval = func(b, user, market, txntype);
  if(bval >= 0.0) {
    b = b.symbol == VIG4_SYMBOL ? eosio::asset(b.amount * std::pow(10.0, 6), VIG10_SYMBOL) : b;
    return b;
  }
  while((b - a).amount > 1 && count++ < 100) {
    c.amount = (a.amount + b.amount) / 2;
    cval = func(c, user, market, txntype);
    aval = func(a, user, market, txntype);
    if(cval == 0.0 || aval <= 0.0)
      break;
    else if(cval < 0)
      b = c;
    else
  ...
  if(cval < 0)
    c = a;
  if(func(b, user, market, txntype) >= 0)
    c = b;
  c = c.symbol == VIG4_SYMBOL ? eosio::asset(c.amount * std::pow(10.0, 6), VIG10_SYMBOL) : c;
  return c;
}
...
// writes messages into the log table, limit number of rows with _config.getLogCount()
