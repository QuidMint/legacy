...
#define BOX_TOKEN_SYMBOL eosio::symbol("BOXK", 6)
...
CONTRACT vigorlending: public eosio::contract {
 private:
  TABLE user_s {
   ...
   public:
    ...
    double rewardrexvot = 0.0;  // last period accumulated rewards paid for all reward types (eg rex, vote proxy, lptoken.defi, lptoken.sx), in dollars, period defined by atimer (for example 86400 sec)
    ...
    double rewardlend = 0;              // accumulated rewards paid for all reward types (eg rex, vote proxy, lptoken.defi, lptoken.sx), in dollars, for the period since last atimer rollover (for example 86400 sec)
    ...
    //0    user         rewardlend          pct LP tokens vs total          double 0.051       5.1%                 // accumulated rewards paid for all reward types (eg rex, vote proxy, lptoken.defi, lptoken.sx), in dollars, for the period since last atimer rollover (for example 86400 sec)
 ...
 public:
  TABLE globalstats {
   ...
   public:
    ...
    uint64_t rexproxy = 0;         // accumulated EOS rewards from REX and proxy voter rewards since last time distribfee to users, units number of EOS
 ...
 private:
  ...
  void takefee(const user_s& user, double txnfeedollars);
  void sellrex(eosio::asset asset);
  void payminer(uint64_t fee);
 ...
 public:
  ...
  // initialvig     number of VIG required to have in collateral to take new debt
  ...
  // gasfee         fee paid to miners calling tick action. units are VIG4 precision. so 1 would mean 0.0001 VIG
  ...
  // mincollat      limit users from taking a new borrow such that the ratio of collateral to debt is above bailoutCR and bailoutupCR, for example 0.05 is bailoutCR + 5%
  ...
  // lpsxcut        percentage of the transaction fees given to depositors of lptoken.sx
#define SEQUENCE                                                                                         \
  ...
  FIELD(mincollat, MinCollat, FIELD_DATA_DOUBLE_TO_INT(uint64_t, 100, 0, UINT64_MAX, 0.05))              \
  ...
  FIELD(datac, DataC, FIELD_DATA_DOUBLE_TO_INT(uint64_t, 1, 0, UINT64_MAX, 1.0))                         \
  ...
  ACTION rewardclaim(eosio::name contract);
