...
void vigorlending::doupdate() {
  ...
  switch(_g.step) {
    ...
    case 2:
      ...
      break;
    case 3:
     if (_g.rexproxy >= _config.getRexSwitch() * 100 && _g.rexproxy % 2 == 0)
      dorewards();
     else if (_config.getDataC() / std::pow(10.0, BOX_TOKEN_SYMBOL.precision()) >= 1.0 && (uint64_t)_config.getDataC() % 2 == 0)
      dorewards2(true);
     else if (_config.getMinCollat() / std::pow(10.0, SYSTEM_TOKEN_SYMBOL.precision()) >= 1.0 && (uint64_t)_config.getMinCollat() % 2 == 0)
      dorewards2(false);
     else {
...
void vigorlending::collectfee() {
  ...
  while(itr != stop_itr && counter++ < _b.batchsize) {
    ...
    double rate = 0.08;
    if (user.getReputation_pct() > 0.75)
      rate = 0.06;
    else if (user.getReputation_pct() > 0.50)
      rate = 0.04;
    else if (user.getReputation_pct() > 0.25)
      rate = 0.02;

    // calculate token swap payment due, denominated in dollars
    double tespay = (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) * (std::pow((1.0 + rate), T) - 1.0);
    double l_tespay = user.l_valueofcol * (std::pow((1.0 + rate), T) - 1.0);

    double tespayrep = (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) * (std::pow((1.0 + user.tesprice), T) - 1.0);
    double l_tespayrep = user.l_valueofcol * (std::pow((1.0 + user.l_tesprice), T) - 1.0);

    double W1 = 0.0;
    double pct = std::min((tespay - W1) / user.valueofcol,1.0);
    auto userCollaterals = getCollaterals(const_cast<user_s&>(user));
    auto vigordacfund = _u.get(_config.getVigorDacFund().value, "VigorDacFund not found");
    for(const auto& userCollateral : asset_container_t(*userCollaterals)) {
      ...
      _u.modify(vigordacfund, get_self(), [&](auto& modified_user) {
        modified_user.getCollaterals() += amt;
      });
    ...
    eosio::asset amta = eosio::asset((tespayrep * std::pow(10.0, VIG10_SYMBOL.precision())) / vigprice, VIG10_SYMBOL);
    eosio::asset l_amta = eosio::asset((l_tespayrep * std::pow(10.0, VIG10_SYMBOL.precision())) / vigprice, VIG10_SYMBOL);
...
void vigorlending::viglifeline(const user_s& user, eosio::asset& vigfees, eosio::asset vigneededa, double vigprice, double vigvol) {
  ...
  std::string message = "VIG lifeline: " + user.usern.to_string() + " VIG lifeline initiated for " + vigneededa.to_string() + " for paying fees. ";
  eosio::asset c;
  eosio::asset d;
        ...
        if(globalInsurances.hasAsset(vigneededa, globalInsuranceAsset)) {
          if(globalInsuranceAsset.amount >= vigneededa.amount) {
            ...
            if(maxBorrowAmount >= 0) {
              ...
              eosio::asset remavail = asset_swap(eosio::asset(vigneededa.amount + maxBorrowAmount,vigneededa.symbol));
...
eosio::symbol vigorlending::symbol_swap(eosio::symbol s){
  if (s == VIG10_SYMBOL) return VIG4_SYMBOL;
  if (s == VIG4_SYMBOL)  return VIG10_SYMBOL;
  if (s.precision() == 0) return eosio::symbol(eosio::symbol(s.code(),1));
  if (s.precision() == 1) return eosio::symbol(eosio::symbol(s.code(),0));
  return s;
}

eosio::asset vigorlending::asset_swap(eosio::asset a){
...
void vigorlending::rewardswap(){
  // sending tokens to swap.defi to get VIG in return. The tokens in vigordacfund collateral are from rewards, transaction fees, and liquidation penalties collected
  // this can be improved by looking up pairs in vigoraclehub
  ...
  for(const auto& userCollateral : asset_container_t(*userCollaterals)) {
    ...
    if (userCollateral.symbol == VIG4_SYMBOL)
      continue;
    ...
    eosio::asset dust = userCollateral - asset_swap(asset_swap(userCollateral));
    _u.modify(user, get_self(), [&](auto& modified_user) {
      userCollaterals -= userCollateral;
    });
    _u.modify(finalreserve, get_self(), [&](auto& modified_user) {
      modified_user.getCollaterals() += dust;
    });
    _g.getCollaterals() -= userCollateral + dust;
  }
  update(*vitr);
  update(*finalreserve);
  updateglobal();
...
// rewards handler
#if defined(TEST_MODE) || defined(LOCAL_MODE)
  ...
  if(lpclaim) {
    if(assetin.symbol.raw() == SYSTEM_TOKEN_SYMBOL.raw() && from == _config.getProxyPay()){
      _g.rexproxy += (assetin.amount / std::pow(10.0, SYSTEM_TOKEN_SYMBOL.precision())) * ((double)_market.get(SYSTEM_TOKEN_SYMBOL.raw()).marketdata.price.back() / pricePrecision); //accumulate proxy vote rewards, denominated as number of dollars
      ...
      _config.set_value(eosio::name("initialvig"), std::to_string((double)(_config.getInitialVig() + (assetin.amount / std::pow(10.0, SYSTEM_TOKEN_SYMBOL.precision())) * ((double)_market.get(SYSTEM_TOKEN_SYMBOL.raw()).marketdata.price.back() / pricePrecision)))); //accumulate LP rewards, denominated as number of dollars
    else if(assetin.symbol.raw() == BOX_TOKEN_SYMBOL.raw())
      _config.set_value(eosio::name("initialvig"), std::to_string((double)(_config.getInitialVig() + (assetin.amount / std::pow(10.0, BOX_TOKEN_SYMBOL.precision())) * ((double)_market.get(BOX_TOKEN_SYMBOL.raw()).marketdata.price.back() / pricePrecision)))); //accumulate LP rewards, denominated as number of dollars
    else if(assetin.symbol.raw() == VIG4_SYMBOL.raw())
      _config.set_value(eosio::name("initialvig"), std::to_string((double)(_config.getInitialVig() + (assetin.amount / std::pow(10.0, VIG4_SYMBOL.precision())) * ((double)_market.get(VIG4_SYMBOL.raw()).marketdata.price.back() / pricePrecision)))); //accumulate LP rewards, denominated as number of dollars
    else
      return;
    _configTable.set(_config, get_self());
    auto vitr = _u.get(_config.getVigorDacFund().value, "VigorDacFund not found");
...
// transaction fee, 10 bps of the amount transacted, maximum 10 dollars, taken from user insurance, collateral, or vigfees, denomination order of preference last for VIG, VIGOR
void vigorlending::takefee(const user_s& user, eosio::asset localAsset) {
  auto market = _market.get(symbol_swap(localAsset.symbol).raw());
  double valueofassetin = (localAsset.amount) / std::pow(10.0, localAsset.symbol.precision()) *
                          ((double)market.marketdata.price.back() / pricePrecision);
  double txnfeedollars = std::min(std::max(valueofassetin * 0.001, 0.0), 10.0);
  double W1 = 0.0;
  double pct = std::min((txnfeedollars - W1) / user.valueofcol,1.0);
  auto userCollaterals = getCollaterals(const_cast<user_s&>(user));
  auto vigordacfund = _u.get(_config.getVigorDacFund().value, "VigorDacFund not found");
  for(const auto& userCollateral : asset_container_t(*userCollaterals)) {
    eosio::asset amt = userCollateral;
    if (amt.symbol.code() == VIG10_SYMBOL.code() || amt.symbol.code() == VIGOR_SYMBOL.code() || amt.symbol.code() == localAsset.symbol.code())
      continue;
    amt.amount *= pct;
    //amt.amount = std::max(amt.amount, (int64_t)1);
    _u.modify(user, get_self(), [&](auto& modified_user) {
  ...
  pct = std::max(std::min((txnfeedollars - W1) / user.valueofins,1.0),0.0);
  if (pct>0.0) {
  auto userInsurances = getInsurances(const_cast<user_s&>(user));
  for(const auto& userInsurance : asset_container_t(*userInsurances)) {
    ...
    _u.modify(vigordacfund, get_self(), [&](auto& modified_user) {
      modified_user.getInsurances() += amt;
    });
    W1 += (amt.amount) / std::pow(10.0, amt.symbol.precision()) * ((double)_market.get(symbol_swap(amt.symbol).raw()).marketdata.price.back() / pricePrecision);
    if (W1 >= txnfeedollars)
      break; 
...
// distribute defibox rewards (BOX and EOS) to all users having LP tokens proportionately
void vigorlending::dorewards2(bool flag) {
  
  eosio::name totalreward = flag ? eosio::name("datac") : eosio::name("mincollat");
  eosio::asset assetin = flag ? eosio::asset(_config.getDataC(), BOX_TOKEN_SYMBOL) : eosio::asset(_config.getMinCollat(), SYSTEM_TOKEN_SYMBOL);
  auto itr = _u.lower_bound(_b.usern.value);
  auto stop_itr = _u.end();
  uint64_t counter = 0;
  std::string message = get_self().to_string() + " has received and distributed LP reward income: " + assetin.to_string() + " ";
  const eosio::name& userntrace = get_self();
  const uint64_t bailoutid = 0;
  eosio::asset a;
  eosio::asset boxk = eosio::asset(0, BOXK1_TOKEN_SYMBOL);
  eosio::asset boxbx = eosio::asset(0, BOXBX1_TOKEN_SYMBOL);
  eosio::asset boxl = eosio::asset(0, BOXL1_TOKEN_SYMBOL);
  eosio::asset boxi = eosio::asset(0, BOXI1_TOKEN_SYMBOL);
  eosio::asset boxgl = eosio::asset(0, BOXGL1_TOKEN_SYMBOL);
  eosio::asset boxfu = eosio::asset(0, BOXFU1_TOKEN_SYMBOL);
  eosio::asset boxa = eosio::asset(0, BOXA1_TOKEN_SYMBOL);
  eosio::asset boxapm = eosio::asset(0, BOXAPM1_TOKEN_SYMBOL);
  eosio::asset boxack = eosio::asset(0, BOXACK1_TOKEN_SYMBOL);
  eosio::asset boxvp = eosio::asset(0, BOXVP1_TOKEN_SYMBOL);
  eosio::asset boxai = eosio::asset(0, BOXAI1_TOKEN_SYMBOL);

  if(_b.usern.value == _u.begin()->usern.value) {
    _b.stepdata = assetin.amount;
    _b.stepdata2 = 0;
    if(_g.getLCollaterals().hasAsset(boxk,a)) _b.stepdata2 += a.amount;
    if(_g.getLCollaterals().hasAsset(boxbx,a))_b.stepdata2 += a.amount;
    if(_g.getLCollaterals().hasAsset(boxl,a)) _b.stepdata2 += a.amount;
    if(_g.getLCollaterals().hasAsset(boxi,a)) _b.stepdata2 += a.amount;
    if(_g.getLCollaterals().hasAsset(boxgl,a)) _b.stepdata2 += a.amount;
    if(_g.getLCollaterals().hasAsset(boxfu,a)) _b.stepdata2 += a.amount;
    if(_g.getLCollaterals().hasAsset(boxa,a)) _b.stepdata2 += a.amount;
    if(_g.getLCollaterals().hasAsset(boxapm,a)) _b.stepdata2 += a.amount;
    if(_g.getLCollaterals().hasAsset(boxack,a)) _b.stepdata2 += a.amount;
    if(_g.getLCollaterals().hasAsset(boxvp,a)) _b.stepdata2 += a.amount;
    if(_g.getLCollaterals().hasAsset(boxai,a)) _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(boxk,a)) _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(boxbx,a)) _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(boxl,a)) _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(boxi,a)) _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(boxgl,a)) _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(boxfu,a)) _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(boxa,a)) _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(boxapm,a)) _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(boxack,a)) _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(boxvp,a)) _b.stepdata2 += a.amount;
    if(_g.getCollaterals().hasAsset(boxai,a)) _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(boxk,a)) _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(boxbx,a)) _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(boxl,a)) _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(boxi,a)) _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(boxgl,a)) _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(boxfu,a)) _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(boxa,a)) _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(boxapm,a)) _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(boxack,a)) _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(boxvp,a)) _b.stepdata2 += a.amount;
    if(_g.getInsurances().hasAsset(boxai,a)) _b.stepdata2 += a.amount;
    eosio::name bailoutuser = _g.bailoutuser;
    eosio::name bailoutupuser = _g.bailoutupuser;
    _b.lastflag = false;
    const auto& lastUserItr = std::find_if(_u.rbegin(), _u.rend(), [&assetin, &bailoutuser, &bailoutupuser](const auto& item) {
      return (( item.getCollaterals().hasAsset(BOXK1_TOKEN_SYMBOL) || 
                item.getCollaterals().hasAsset(BOXBX1_TOKEN_SYMBOL) ||
                item.getCollaterals().hasAsset(BOXL1_TOKEN_SYMBOL) ||
                item.getCollaterals().hasAsset(BOXI1_TOKEN_SYMBOL) ||
                item.getCollaterals().hasAsset(BOXGL1_TOKEN_SYMBOL) ||
                item.getCollaterals().hasAsset(BOXFU1_TOKEN_SYMBOL) ||
                item.getCollaterals().hasAsset(BOXA1_TOKEN_SYMBOL) ||
                item.getCollaterals().hasAsset(BOXAPM1_TOKEN_SYMBOL) ||
                item.getCollaterals().hasAsset(BOXACK1_TOKEN_SYMBOL) ||
                item.getCollaterals().hasAsset(BOXVP1_TOKEN_SYMBOL) ||
                item.getCollaterals().hasAsset(BOXAI1_TOKEN_SYMBOL) ||
                item.getInsurances().hasAsset(BOXK1_TOKEN_SYMBOL) ||
                item.getInsurances().hasAsset(BOXBX1_TOKEN_SYMBOL) ||
                item.getInsurances().hasAsset(BOXL1_TOKEN_SYMBOL) ||
                item.getInsurances().hasAsset(BOXI1_TOKEN_SYMBOL) ||
                item.getInsurances().hasAsset(BOXGL1_TOKEN_SYMBOL) ||
                item.getInsurances().hasAsset(BOXFU1_TOKEN_SYMBOL) ||
                item.getInsurances().hasAsset(BOXA1_TOKEN_SYMBOL) ||
                item.getInsurances().hasAsset(BOXAPM1_TOKEN_SYMBOL) ||
                item.getInsurances().hasAsset(BOXACK1_TOKEN_SYMBOL) ||
                item.getInsurances().hasAsset(BOXVP1_TOKEN_SYMBOL) ||
                item.getInsurances().hasAsset(BOXAI1_TOKEN_SYMBOL)) && 
                !(item.usern == bailoutuser || item.usern == bailoutupuser));
    });
    if(lastUserItr == _u.rend())
      _b.lastinsurer = eosio::name(0);
    else
  ...
  if(_b.lastinsurer == eosio::name(0)) {  // no user has any LP's, give rewards to finalreserve
    auto lastUseritr = _u.find(_config.getFinalReserve().value);
    const auto& lastUser = *lastUseritr;
    while(itr != stop_itr && counter++ < _b.batchsize)
      itr++;
    if(itr == _u.end()) {
      _u.modify(lastUser, get_self(), [&](auto& modified_user) {
        getInsurances(modified_user) += assetin;
        modified_user.rewardrexvot = flag ? modified_user.rewardrexvot : modified_user.rewardrexvot + (assetin.amount) / std::pow(10.0, assetin.symbol.precision());
      });
      eosio::action(eosio::permission_level {get_self(), eosio::name("active")}, get_self(), eosio::name("configure"), std::make_tuple(totalreward,std::to_string(0.0))).send();
      tracetable(__func__, __LINE__, userntrace, bailoutid, message);
      const auto rank = _u.get_index<"reputation"_n>();
      _b.usern = rank.begin()->usern;
      _g.step = 4;
    ...
    while(itr != stop_itr && counter++ < _b.batchsize) {
      ...
      userEOSCol = 0;
      userEOSIns = 0;
      if(user.getCollaterals().hasAsset(boxk,a)) userEOSCol += a.amount;
      if(user.getCollaterals().hasAsset(boxbx,a))userEOSCol += a.amount;
      if(user.getCollaterals().hasAsset(boxl,a)) userEOSCol += a.amount;
      if(user.getCollaterals().hasAsset(boxi,a)) userEOSCol += a.amount;
      if(user.getCollaterals().hasAsset(boxgl,a)) userEOSCol += a.amount;
      if(user.getCollaterals().hasAsset(boxfu,a)) userEOSCol += a.amount;
      if(user.getCollaterals().hasAsset(boxa,a)) userEOSCol += a.amount;
      if(user.getCollaterals().hasAsset(boxapm,a)) userEOSCol += a.amount;
      if(user.getCollaterals().hasAsset(boxack,a)) userEOSCol += a.amount;
      if(user.getCollaterals().hasAsset(boxvp,a)) userEOSCol += a.amount;
      if(user.getCollaterals().hasAsset(boxai,a)) userEOSCol += a.amount;
      if(user.getInsurances().hasAsset(boxk,a))userEOSIns += a.amount;
      if(user.getInsurances().hasAsset(boxbx,a))userEOSIns += a.amount;
      if(user.getInsurances().hasAsset(boxl,a)) userEOSIns += a.amount;
      if(user.getInsurances().hasAsset(boxi,a)) userEOSIns += a.amount;
      if(user.getInsurances().hasAsset(boxgl,a)) userEOSIns += a.amount;
      if(user.getInsurances().hasAsset(boxfu,a)) userEOSIns += a.amount;
      if(user.getInsurances().hasAsset(boxa,a)) userEOSIns += a.amount;
      if(user.getInsurances().hasAsset(boxapm,a)) userEOSIns += a.amount;
      if(user.getInsurances().hasAsset(boxack,a)) userEOSIns += a.amount;
      if(user.getInsurances().hasAsset(boxvp,a)) userEOSIns += a.amount;
      if(user.getInsurances().hasAsset(boxai,a)) userEOSIns += a.amount;
      if(userEOSCol > 0 && _b.stepdata > 0) {
        amt.amount = assetin.amount * (double)userEOSCol / _b.stepdata2;
        amt.amount = std::max(amt.amount, (int64_t)1);
        _u.modify(user, get_self(), [&](auto& modified_user) {
          if(user == lastUser && userEOSIns == 0)
            amt.amount = _b.stepdata;
          getCollaterals(modified_user) += amt;
          modified_user.rewardrexvot = flag ? modified_user.rewardrexvot : modified_user.rewardrexvot + (amt.amount) / std::pow(10.0, amt.symbol.precision());
        });
        _b.stepdata -= amt.amount;
      }
...
void vigorlending::exassetout(const eosio::name& usern, const eosio::asset& assetout, const std::string& memo, const bool& exec, const bool partialupdate, const uint64_t& step, double& totalval, double& totalvalbefore) {
  ...
  if(memo == "borrow" && assetout.symbol == VIGOR_SYMBOL) {
    ...
    eosio::check(user.valueofcol >= mincollat * 0.998 * (debt.amount / std::pow(10.0, debt.symbol.precision()) + lifelinebuffer),
                 "Max allowable borrow " + eosio::asset(std::pow(10.0, debt.symbol.precision()) * std::max((user.valueofcol / (mincollat)) - ((debt.amount - localAssetOut.amount) / std::pow(10.0, debt.symbol.precision())) - lifelinebuffer, 0.0), VIGOR_SYMBOL).to_string() +
                   ". Collateral Ratio limit " + std::to_string(mincollat));
    ...
    if(memo == "savings") {
      ...
      if(!(valueofcol >= mincollat * 0.998 * (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())))) {
        eosio::asset x = eosio::asset(std::pow(10.0, collateral.symbol.precision()) * (std::min((collateral.amount / std::pow(10.0, collateral.symbol.precision())) * ((double)market.marketdata.price.back() / pricePrecision), std::max((user.valueofcol - mincollat * (user.debt.amount / std::pow(10.0, user.debt.symbol.precision()))), 0.0)) / ((double)market.marketdata.price.back() / pricePrecision)), collateral.symbol);
      ...
      if(!(user.l_valueofcol + valueofasset + lifelinebuffer <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())) / (mincollatup * 0.998))) {
        eosio::asset x = eosio::asset(std::pow(10.0, localAssetOut.symbol.precision()) * (std::max(((user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())) / mincollatup - user.l_valueofcol), 0.0) / ((double)market.marketdata.price.back() / pricePrecision)), localAssetOut.symbol);
        eosio::check(false, "Max allowable crypto borrow " + x.to_string() +
                              ". Collateral Ratio limit " + std::to_string(mincollatup));
      }
      ...
      double l_tespay = (user.l_valueofcol + valueofasset) * (std::pow(1.0 + _config.getMaxTesPrice(), _config.getvigLifeLine() / 360.0) - 1.0);
      eosio::asset tespayvigor = eosio::asset(std::max((int64_t)(tespay * std::pow(10.0, VIGOR_SYMBOL.precision())), (int64_t)1), VIGOR_SYMBOL);
      eosio::asset l_tespayvigor = eosio::asset(std::max((int64_t)(l_tespay * std::pow(10.0, VIGOR_SYMBOL.precision())), (int64_t)1), VIGOR_SYMBOL);
      eosio::asset amta = eosio::asset(uint64_t(((tespayvigor.amount / std::pow(10.0, VIGOR_SYMBOL.precision())) / vigprice) * std::pow(10.0, VIG10_SYMBOL.precision())), VIG10_SYMBOL);
      eosio::asset l_amta = eosio::asset(uint64_t(((l_tespayvigor.amount / std::pow(10.0, VIGOR_SYMBOL.precision())) / vigprice) * std::pow(10.0, VIG10_SYMBOL.precision())), VIG10_SYMBOL);
      ...
      if(!exec && user.vigfees <= 0) {
        ...
        if(((_g.getInsurances().hasAsset(vigfees, ginsurance) && (ginsurance.amount - lr.amount >= vigneededa.amount)) &&
            ((((user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) + overcol * vigneeded) * mincol <= user.valueofcol) ||
             ((user.l_valueofcol + vigneeded + valueofasset) * mincolup <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision()))))))
          lifeline = true;
        eosio::check(lifeline, "Zero balance not allowed for VIG Fees. Please deposit VIG, it is needed to pay the fees over time.");
      }
      ...
      // if overcollateralization is C then leverage L = 1 / ( 1 - ( 1 / C ) )
      eosio::check(user.l_valueofcol * mincollatup * 0.998 <= (l_debt.amount / std::pow(10.0, l_debt.symbol.precision())),
                   "Max allowable collateral withdraw " + eosio::asset(std::pow(10.0, l_debt.symbol.precision()) * std::max(((user.l_debt.amount / std::pow(10.0, l_debt.symbol.precision())) - (user.l_valueofcol * mincollatup)), 0.0), VIGOR_SYMBOL).to_string() +
                     ". Collateral Ratio limit " + std::to_string(mincollatup));
...
void vigorlending::pricing(const user_s& user) {
  ...
  double T = 1.0;
  double calib = 0.7; //calibration paramter, premultiplier to tenor in the pricing model
  double d = ((std::log(user.valueofcol / (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())))) + (-std::pow(ivol, 2) / 2.0) * T * calib) / (ivol * std::sqrt(T * calib));

  double tesprice = 0.0;
  if(user.debt.amount > 0)
    tesprice = std::min(std::max(_config.getMinTesPrice(),
...
void vigorlending::l_pricing(const user_s& user) {
  ...
  double T = 1.0;
  double calib = 0.7; //calibration paramter, premultiplier to tenor in the pricing model
  double d = ((std::log(user.l_valueofcol / (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())))) + (-std::pow(l_ivol, 2) / 2.0) * T * calib) / (l_ivol * std::sqrt(T * calib));

  double l_tesprice = 0.0;
  if(user.l_valueofcol > 0)
    l_tesprice =  std::min(std::max(_config.getMinTesPrice(),
...
void vigorlending::distribfee() {
  ...
  if(_b.usern.value == _u.begin()->usern.value) {
    ...
    _b.stepdata = (_g.fee.amount + _g.l_fee.amount + std::pow(10.0, _g.fee.symbol.precision()) * (_g.rexproxy/vigprice + _config.getInitialVig()/vigprice));
  }
  ...
  while(itr != stop_itr && counter++ < _b.batchsize) {
    if(_b.lastflag || (_g.fee.amount + _g.l_fee.amount + std::pow(10.0, _g.fee.symbol.precision()) * (_g.rexproxy/vigprice + _config.getInitialVig()/vigprice) <= 0)) {
      itr++;
      continue;
    }
    ...
    uint64_t rex_lp_rewards = std::pow(10.0, _g.fee.symbol.precision()) * (user.rewardrexvot * _g.rexproxy/vigprice + user.rewardlend * _config.getInitialVig()/vigprice);

    viga.amount += rex_lp_rewards;
    ...
    if(viga.amount > 0 && user.usern.value != _config.getFinalReserve().value) {
      double pct = 1.0;                                                                       // amount of VIG in the user insurance as a percentage of the total dollar value of their insurance
      double oc = 1.0 + 1.0 * vigvol / sqrt(12);                                              // +2.0 * vigvol / sqrt(12); // overcollateralize the VIGOR borrow by this amount, enough collateral to survive a one monthly standard deviation move of VIG price
    ...
    _u.modify(user, get_self(), [&](auto& modified_user) {
      if(user.usern == _config.getFinalReserve()) {
        getInsurances(modified_user) += viga;  // reserve receives the fee into insurance and also adds to global insurance, the reserve does not pay any fees so it needs no VIG in vigfees.
      ...
      double days_since_anniv = (3600.0 + eosio::current_time_point().sec_since_epoch() - (modified_user.getAnniv().sec_since_epoch() - _config.getRepAnniv())) / eosio::days(1).to_seconds();
      uint64_t rep = std::pow(10.0, _g.fee.symbol.precision()) * std::sqrt(std::pow(modified_user.getVig_since_anniv().amount / std::pow(10.0, _g.fee.symbol.precision()), 2.0) + (i_reputation(viga - eosio::asset(rex_lp_rewards, viga.symbol) + pctamta).amount / std::pow(10.0, _g.fee.symbol.precision())));
      modified_user.setRep_lag0(eosio::asset(rep / std::sqrt(days_since_anniv), _g.fee.symbol));
      modified_user.setVig_since_anniv(eosio::asset(rep, _g.fee.symbol));
...
void vigorlending::update(const user_s& user) {
  ...
  for(const auto& collateral : *user.getCollaterals()) {
    ...
    if (symbolIterator->contract.value == eosio::name("lptoken.defi").value)
      rewardlend += valueofcol;
    if (symbolIterator->sym.raw() == SYSTEM_TOKEN_SYMBOL.raw())
      rewardrexvot += valueofcol;
  }
  rewardlend /= _config.getDataC();
  rewardrexvot /= _config.getMinCollat();
  ...
  _u.modify(user, get_self(), [&](auto& modified_user) {  // Update value of collateral
    ...
    if(modified_user.rewardlend != rewardlend)
      modified_user.rewardlend = rewardlend;
    if(modified_user.rewardrexvot != rewardrexvot)
      modified_user.rewardrexvot = rewardrexvot;
  });
}
...
void vigorlending::updateglobal() {
  ...
  auto market = _market.get(VIGOR_SYMBOL.raw());
  if(_g.savingsscale != std::max(std::min(std::pow(1.0 / ((double)market.marketdata.price.back() / pricePrecision), 4.0), 1.65), 0.33))
    _g.savingsscale = std::max(std::min(std::pow(1.0 / ((double)market.marketdata.price.back() / pricePrecision), 4.0), 1.65), 0.33);
  ...
  if(_g.l_valueofcol != l_valueofcol)
    ...
    _config.set_value(eosio::name("datac"), std::to_string(datac));
    _config.set_value(eosio::name("mincollat"), std::to_string(mincollat));
    _configTable.set(_config, get_self());
...
void vigorlending::bailout(const eosio::name& usern) {
  ...
  if(_b.usern.value == _u.begin()->usern.value) {
    ...
    if(vigordaccut < 1.0) {
      ...
      for(const auto& userCollateral : asset_container_t(*userCollaterals)) {
        eosio::asset amt(userCollateral.amount, userCollateral.symbol);
        amt.amount *= ( 1.0 - vigordaccut ) / 2.0;
        auto& vigordacfund = _u.get(_config.getVigorDacFund().value, "VigorDacFund not found");
        _u.modify(vigordacfund, get_self(), [&](auto& modified_user) {
          modified_user.getCollaterals() += amt;
        });
      ...
      liqvalueofcol *= ( vigordaccut + ( ( 1.0 - vigordaccut ) / 2.0 ) );
    }
    if (!_b.assetcont.empty()) {
