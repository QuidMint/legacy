...
void vigorlending::liquidate(const eosio::name& usern) {
  ...
  eosio::check(user.valueofins == 0, "User must first withdraw all tokens from lending/insurance");
  ...
  // schaction(get_self(), eosio::name("doupdate"), std::make_tuple(), _config.getAssetOutTime(), _config.getMineBuffer(), _g.ac % _b.batchsize == 0 ? 15 * (_g.ac / _b.batchsize) + 2 : 15 * (1 + _g.ac / _b.batchsize) + 2);
  const eosio::name& userntrace = usern;
  const uint64_t bailoutid = 0;
...
void vigorlending::viglifeline(const user_s& user, eosio::asset& vigfees, eosio::asset vigneededa, double vigprice, double vigvol) {
  ...
  double overcol = (_config.getBailoutupCR() / 100) + 2.0 * vigvol / sqrt(12);  // overcollateralize the VIG borrow by this amount, enough collateral to survive a 2 * monthly standard deviation move of VIG price
  ...
  if(user.getInsurances().hasAsset(_g.fee, c) && c.amount >= vigneededa.amount && _g.getInsurances().hasAsset(_g.fee, globalInsurance) && globalInsurance.amount >= vigneededa.amount) {
  ...
  } else if(user.getCollaterals().hasAsset(_g.fee, d) && d.amount >= vigneededa.amount && (user.valueofcol - vigneeded >= (0.02 + (_config.getBailoutCR() / 100)) * (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())))) {
    ...
    if(!debtCeiling) {
      double mincol = (_config.getBailoutCR() / 100);
      double mincolup = (_config.getBailoutupCR() / 100);
      if((((user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) + overcol * vigneeded) * mincol <= user.valueofcol) ||
         ((user.l_valueofcol + vigneeded) * mincolup <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())))) {
        ...
        if(globalInsurances.hasAsset(vigneededa, globalInsuranceAsset)) {
          if(globalInsuranceAsset.amount >= vigneededa.amount) {
            ...
            if(maxBorrowAmount >= 0) {
              if(!((user.l_valueofcol + vigneeded) * mincolup <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision()))) ||
                 ((user.valueofcol - ((user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) + overcol * vigneeded) * mincol) >
                  ((user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())) - (user.l_valueofcol + vigneeded) * mincolup))) {
  ...
  switch(_config.getExecType()) {
    ...
    case 2: {  // mining
      ...
      if(t0 || t1 || t2 || t3 || t4 || t5 || t6) {                                                                                 // full update
        ...
        // schaction(get_self(), eosio::name("doupdate"), std::make_tuple(), 0, _config.getMineBuffer(), _g.ac % _b.batchsize == 0 ? 15 * (_g.ac / _b.batchsize) + 2 : 15 * (1 + _g.ac / _b.batchsize) + 2);
...
void vigorlending::paybacktok(const user_s& user, eosio::asset assetin) {
  ...
  if(assetin.symbol == VIG10_SYMBOL && l_collateral.amount < 1000000 && assetin.amount == 1000000) {  // user paying VIG4 0.0001 VIG, to payoff dust loan of VIG10
    eosio::asset dust = assetin - l_collateral;
    assetin = l_collateral;
    _u.modify(user, get_self(), [&](auto& modified_user) {
      modified_user.vigfees += dust.amount;  // paid too much, a dust amount, send to user vigfees
      _g.vigfees += dust.amount;
    });
  }

  eosio::check(l_collateral >= assetin, "Payback amount too high. " + assetin.to_string() + " > " + l_collateral.to_string() + " owner: " + userntrace.to_string());
...
void vigorlending::exassetout(const eosio::name& usern, const eosio::asset& assetout, const std::string& memo, const bool& exec, const bool partialupdate, const uint64_t& step, double& totalval, double& totalvalbefore) {
  ...
  double mincollat = (_config.getBailoutCR() / 100) + 0.05;
  double mincollatup = (_config.getBailoutupCR() / 100) + 0.05;
  if(memo == "borrow" && assetout.symbol == VIGOR_SYMBOL) {
    ...
    if(!exec && user.vigfees <= 0) {
      ...
      if((user.getCollaterals().hasAsset(_g.fee, c) && c.amount >= vigneededa.amount && (user.valueofcol - vigneeded >= (0.02 + mincol) * ((user.debt.amount + localAssetOut.amount) / std::pow(10.0, user.debt.symbol.precision())))) ||
      ...
      if(((_g.getInsurances().hasAsset(vigfees, ginsurance) && (ginsurance.amount - lr.amount >= vigneededa.amount)) &&
          (((((user.debt.amount + localAssetOut.amount) / std::pow(10.0, user.debt.symbol.precision())) + overcol * vigneeded) * mincol <= user.valueofcol) ||
           ((user.l_valueofcol + vigneeded) * mincolup <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision()))))))
        lifeline = true;
    ...
    if(memo == "savings") {
      ...
      userlcol -= localAssetOut;
      if(!(user.l_valueofcol + valueofassetout + lifelinebuffer <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())) / (std::max(1.0 + (l_volcol - 0.6), 1.0) * mincollatup * 0.998))) {
        eosio::asset x = bisection(eosio::asset(0, localAssetOut.symbol), eosio::asset(std::pow(10.0, localAssetOut.symbol.precision()) * (std::max(((user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())) / mincollatup - user.l_valueofcol), 0.0) / ((double)market.marketdata.price[0] / pricePrecision)), localAssetOut.symbol), user, false);
        userlcol += x;
        ...
        eosio::check(false, "Max allowable crypto borrow " + x.to_string() +
                              ". Collateral Ratio limit " + std::to_string(std::max(1.0 + (l_volcol - 0.6), 1.0) * mincollatup));
      }
      ...
      if(!exec && user.vigfees <= 0) {
        ...
        double mincol = (_config.getBailoutCR() / 100);
        double mincolup = (_config.getBailoutupCR() / 100);
        if((user.getCollaterals().hasAsset(_g.fee, c) && c.amount >= vigneededa.amount && (user.valueofcol - vigneeded >= (0.02 + mincol) * (user.debt.amount / std::pow(10.0, user.debt.symbol.precision())))) ||
           (user.getInsurances().hasAsset(_g.fee, d) && d.amount >= vigneededa.amount && _g.getInsurances().hasAsset(_g.fee, globalInsurance) && globalInsurance.amount >= vigneededa.amount))
          lifeline = true;
        if(((_g.getInsurances().hasAsset(vigfees, ginsurance) && (ginsurance.amount - lr.amount >= vigneededa.amount)) &&
            ((((user.debt.amount / std::pow(10.0, user.debt.symbol.precision())) + overcol * vigneeded) * mincol <= user.valueofcol) ||
             ((user.l_valueofcol + vigneeded + valueofassetout) * mincolup <= (user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision()))))))
          lifeline = true;
        eosio::check(lifeline, "Zero balance not allowed for VIG Fees. Please deposit VIG, it is needed to pay the fees over time.");
      }
      ...
      // if overcollateralization is C then leverage L = 1 / ( 1 - ( 1 / C ) )
      eosio::check(user.l_valueofcol * std::max(1.0 + (user.l_volcol - 0.6), 1.0) * mincollatup * 0.998 <= (l_debt.amount / std::pow(10.0, l_debt.symbol.precision())),
                   "Max allowable collateral withdraw " + eosio::asset(std::pow(10.0, l_debt.symbol.precision()) * std::max(((user.l_debt.amount / std::pow(10.0, l_debt.symbol.precision())) - (user.l_valueofcol * std::max(1.0 + (user.l_volcol - 0.6), 1.0) * mincollatup)), 0.0), VIGOR_SYMBOL).to_string() +
                     ". Collateral Ratio limit " + std::to_string(std::max(1.0 + (user.l_volcol - 0.6), 1.0) * mincollatup));
...
void vigorlending::recap(const user_s& user, const user_s& insurerUser, double insurerpcts, double& sumpcts, bool lastInsurer) {
  ...
  for(const auto& userCollateral : asset_container_t(*userCollaterals)) {
    ...
    auto market = _market.get(amt.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : amt.symbol.raw());
    userValueofcol += amt.amount / std::pow(10.0, amt.symbol.precision()) * ((double)market.marketdata.price[0] / pricePrecision);
  }
  ...
  for(const auto& insurance : asset_container_t(*insurerInsurances)) {
    ...
    if(blocked.amount > 0 && insurerUser.usern != _config.getFinalReserve()) {
      ...
      auto market = _market.get(blocked.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : blocked.symbol.raw());
      double debtpct = std::min((((blocked.amount) / std::pow(10.0, blocked.symbol.precision())) * ((double)market.marketdata.price[0] / pricePrecision)) / (insurerUser.debt.amount / std::pow(10.0, insurerUser.debt.symbol.precision())), 1.0);     // blocked amt as a pct of insurer debt
      double blockedpct = std::min((insurerUser.debt.amount / std::pow(10.0, insurerUser.debt.symbol.precision())) / (((blocked.amount) / std::pow(10.0, blocked.symbol.precision())) * ((double)market.marketdata.price[0] / pricePrecision)), 1.0);  // insurer debt as a pct of the blocked amt
...
void vigorlending::recapup(const user_s& user, const user_s& insurerUser, double insurerlpcts, double& sumpcts, bool lastInsurer) {
  ...
  for(const auto& userLCollateral : asset_container_t(*userLCollaterals)) {
    ...
    auto market = _market.get(amt.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : amt.symbol.raw());
    W1 += (amt.amount) / std::pow(10.0, amt.symbol.precision()) * ((double)market.marketdata.price[0] / pricePrecision);
    eosio::asset userInsurance;
    if(insurerUser.getInsurances().hasAsset(amt, userInsurance)) {
      ...
      W1 -= (amti.amount) / std::pow(10.0, amti.symbol.precision()) * ((double)market.marketdata.price[0] / pricePrecision);                   //adjusted value of the share of l_collateral to include cancelling some of the borrow
  ...
  for(const auto& insurance : asset_container_t(*insurerUser.getInsurances())) {
    ...
    if(blocked.amount > 0) {
      ...
      auto market = _market.get(amt.symbol == VIG10_SYMBOL ? VIG4_SYMBOL.raw() : amt.symbol.raw());
      double blockedval = (((blocked.amount) / std::pow(10.0, blocked.symbol.precision())) * ((double)market.marketdata.price[0] / pricePrecision));
...
double vigorlending::func(eosio::asset x, const user_s& user, const market_s& market, bool txntype) {
  ...
  if(txntype) {
    ...
    userlcol -= x;
    return ((user.l_debt.amount / std::pow(10.0, user.l_debt.symbol.precision())) / (std::max(1.0 + (l_volcol - 0.6), 1.0) * mincollatup)) - l_valueofcol;
  }
}
