...
swapDefi::swapDefi( eosio::name receiver, eosio::name code, eosio::datastream<const char *> ds )
...
void swapDefi::config() {
}

[[eosio::action]]
void swapDefi::addpair(uint64_t id,token token0, token token1, eosio::asset reserve0, eosio::asset reserve1, uint64_t liquidity_token, double price0_last, double price1_last, uint64_t price0_cumulative_last, uint64_t price1_cumulative_last, eosio::time_point_sec block_time_last) {

  require_auth(get_self());

  pairs pairs(_self, _self.value);
  //datapoints_table dstore(_self, pair.name.value);

  auto itr = pairs.find(id);
  ...
  pairs.emplace(_self, [&](auto& a) {
         ...
         a.reserve0=reserve0,
         a.reserve1=reserve1,
         a.liquidity_token=liquidity_token;
         a.price0_last = price0_last;
         a.price1_last = price1_last;
         a.price0_cumulative_last = price0_cumulative_last;
         a.price1_cumulative_last = price1_cumulative_last;
  });
  } else {
