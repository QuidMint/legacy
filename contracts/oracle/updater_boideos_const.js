...
const url = "https://api.newdex.io/v1/price?symbol=boidcomtoken-boid-eos";
			...
			if (results.data && results.data.data.price){
				...
				eos.contract(oracleContract)
					.then((contract) => {
						contract.write({
								owner: owner,
								quotes:[{value: parseInt(Math.round(0.0000364 * 10000000)), pair: "boideos"}]
								//quotes:[{value: parseInt(Math.round(results.data.data.price * 10000000)), pair: "boideos"}]
							},
							{
